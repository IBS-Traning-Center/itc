...
$html.='</div>';?>
    ...
    <?$html.='<div class="video-wrap"><h3>Видео</h3>'?>
					...
					<?foreach ($arResult["VIDEO"] as $key=>$arVideo) {?>
                       <?$html.='<div style="margin-bottom: 14px;" class="cert-item-text"><a target="_blank" href="'.$arVideo["LINK"].'" class="underline">'.UTF($arVideo["NAME"]).'</a></div>';?>
						<?$t++?>
...
';
	...
	$mpdf->SetHTMLHeader('<div style="text-align: right;"><img src="/images/logo_new_pdf.jpg" alt="" title="" border="0"  /></div>');
	$mpdf->SetHTMLFooter('<table class="footer"><tr><td>По всем вопросам обращайтесь <a style="color: #a1a1a1" href="mailto:'.EMAIL_ADDRESS.'" class="underline">'.EMAIL_ADDRESS.'</a></td><td style="width: 34%;text-align: center;">{PAGENO}</td><td style="text-align: right;">Версия {DATE j.m.Y}</td></tr></table>');
	$mpdf->WriteHTML($html);
	...
	$info='	<h4>Об учебном центре Luxoft Training</h4>
			<p><b>Luxoft Training</b> – лидер в области обучения и консалтинга по важнейшим дисциплинам Software Engineering. Учебный центр существует с 2007 г. и предлагает более 150 курсов, тренингов и учебных программ. Обучение проводят более 120 профессиональных тренеров – экспертов-практиков. 
			За это время в Luxoft Training прошли обучение сотрудники ведущих российских и международных компаний. Эффективность обучения подтверждается многочисленными положительными отзывами наших клиентов:</p>
			...
			<p><b>НОРДЕА БАНК</b>: «Эксперт по управлению и коммуникациями УЦ Luxoft Дмитрий Башакин на протяжении всего тренинга удерживал внимание аудитории благодаря интересному диалогу, примерам из практики и легкой подаче материала...  Выражаем благодарность за хорошую организацию обучения, высокий профессионализм экспертов и качественно разработанные курсы. Надеемся на дальнейшее сотрудничество с Учебным Центром Luxoft».</p>
			...
			<p>Обучение проходит в открытом, корпоративном и онлайн-формате. Центры обучения расположены в Москве, Санкт-Петербурге, Омске, Киеве, Одессе, Днепропетровске.<br/>Обучение может быть организовано на территории заказчика, с учетом требований производственного процесса.</p>
		';
	$mpdf->AddPage();
	$mpdf->WriteHTML($info);
	 ...
	 $arResult["PROPERTIES"] = $ob->GetProperties();
        ...
        if (($course_tran_desc_html_type=="text") or ($course_tran_desc_html_type=="TEXT")) {
	      ...
	      if (($course_required_html_type=="text") or ($course_required_html_type=="TEXT")){
	      	$course_required = UTF(nl2br($course_required_html_text));
	      } else {
	      	$course_required = UTF($arResult['PROPERTIES']['course_req_new']['~VALUE']['TEXT']);
	      }
	...
	<? if(strlen($course_required)>1)  {  ?>
     <?$html.="<div class=\"item-section\">
		...
		<? } else {?>
		<?$html.="<div class=\"indent\">{$course_required}</div><br />"?>
		<? }?>
     <?$html.="</div>"?>
	<? } ?>
...
$mpdf->SetHTMLHeader('<div style="text-align: right !important; width: 100%;"><img src="/images/logo_new_pdf.jpg" alt="" title="" border="0"  style="float: right" /></div>');
$mpdf->SetHTMLFooter('<table class="footer"><tr><td>По всем вопросам обращайтесь <a taget="_blank" style="color: #a1a1a1" href="mailto:'.EMAIL_ADDRESS.'" class="underline">'.EMAIL_ADDRESS.'</a></td><td style="width: 34%;text-align: center;">{PAGENO}</td><td style="text-align: right;">Версия {DATE j.m.Y}</td></tr></table>');
$mpdf->WriteHtml($html);
