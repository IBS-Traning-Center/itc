...
if (!$sftp->login($ftp_user_name, $ftp_user_pass)) {
	...
	echo "</pre>";
	$file=$sftp->get('/Reports/Custom_reports/ToBitrix_48067_05_24_2017_01_42.TXT');
 		...
 		if ($arItem["11"]!="Completed") {
			if ($lastCode!=$arItem[9]) {
				if (strlen($arItem[14])>0) {
					...
					if (!is_array($arResultArray[$arItem[14]][$arItem[9]])) {
						$arResultArray[$arItem[14]][$arItem[9]]=$arItem;
					}
					$arResultArray[$arItem[14]][$arItem[9]]["DATES"][]=$dates;
					$arResultArray[$arItem[14]][$arItem[9]]["DURATION"]=$arResultArray[$arItem[14]][$arItem[9]]["DURATION"]+$arItem[16];
				} else {
					$dates=array($arItem[6], $arItem[7]);
					//$arResultArray[$arItem[15]]=array();
					if (!is_array($arResultArray[$arItem[15]][$arItem[9]])) {
						$arResultArray[$arItem[15]][$arItem[9]]=$arItem;
					}
					$arResultArray[$arItem[15]][$arItem[9]]["DATES"][]=$dates;
					$arResultArray[$arItem[15]][$arItem[9]]["DURATION"]=$arResultArray[$arItem[15]][$arItem[9]]["DURATION"]+$arItem[16];
				}
				$lastCode=$arItem[9];
			} else {
				if (strlen($arItem[14])>0) {
		...
		if ($city=="Omsk" || $city=="St.-Petersburg" || $city=="Moscow" || $city=="Kiev" || $city=="Odessa" || $city=="Dnepropetrovsk" || $city=="Virtual")  {
			foreach ($arSchedule as $code=>$arCourse) {
				...
				if ($ob = $res->GetNextElement())
				{
					...
					$arPROP["schedule_duration"]=intval($arCourse["DURATION"]/60);
					if ($arCourse[11]!="N/A") {
			...
			if (strlen($city)>0) {
				foreach ($arSchedule as $code=>$arCourse) {
					...
					if ($ob = $res->GetNextElement())
					{
						...
						if ($city=="Bucharest") {
							$arPROP["LANG"]=44384;
						} elseif ($city=="Krakow" || $city=="Wroclaw") {
							$arPROP["LANG"]=44383;
						} 
						$arPROP["LMS_ID"]=$code;
						if ($arCourse[11]!="N/A") {
