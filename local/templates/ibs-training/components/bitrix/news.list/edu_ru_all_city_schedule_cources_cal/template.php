...
$(document).ready(function() {
	...
	$("a.add-to-basket").click(function(){
		...
		$(".basketSmall").fadeOut("slow");
	   	...
	   	//pageTracker._trackEvent('Order', 'Put
	   	oBasket', id_record);
	...
	while (list($key, $value) = each($arValueOfCourses)) {
        ...
        <div class="timetable-item" itemscope itemtype="http://data-vocabulary.org/Event" >
            <div class="frame no-y-padding clearfix timetable-flex">
                ...
                <?}?>
<a style="margin-bottom: 0;" class="sign-in small" <?if ($value['schedule_id']=="64888") {?>href="/scrum-master/" <?} elseif (strlen($value["landing_link"])==0) {?>href="/kurs/<?=$value['XML']?>.html<? if ($value["show_basket"] === "Y"){?>?ID_TIME=<?=$value['schedule_id']?>#register<? } ?>"<?} else {?>href="<?=$value["landing_link"]?>"<?}?>>Записаться</a></div>
					</div>
