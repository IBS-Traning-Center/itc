...
if (intval($arResult["PROPERTIES"]["FIRST_PRICE"]["VALUE"])>0 && intval($arResult["PROPERTIES"]["SECOND_PRICE"]["VALUE"])>0 && intval($arResult["PROPERTIES"]["THIRD_PRICE"]["VALUE"])>0 && intval($arResult["PROPERTIES"]["FOURTH_PRICE"]["VALUE"])>0) {
	...
	$arResult["ETAPS_COUNT"]=1;
		...
		if (strlen($arResult["COURSE"]["course_topics"])==0) {
			  $course_topics_html_text = $arCourse['PROPERTIES']['course_top_html']['VALUE']['TEXT'];
			  $course_topics_html_type = $arCourse['PROPERTIES']['course_top_html']['VALUE']['TYPE'];
			  if (($course_topics_html_type=="text") or ($course_topics_html_type=="TEXT")) {
				...
				$arResult["COURSE"]["course_topics"]  = $arCourse['PROPERTIES']['course_top_html']['~VALUE']['TEXT'];
			  }
          }
	...
	if(count($arResult["PROPERTIES"]["SCHOOL_STRUCTURE"]["VALUE"]) > 0) {

		foreach ($arResult["PROPERTIES"]["SCHOOL_STRUCTURE"]["VALUE"] as $arItem) 
		{
			...
			{
				$arFields = $ob->GetFields();
				$arResult["SCHOOL_STRUCTURE"][$arItem] = $arFields;
				$arResult["PROPERTIES"]["SCHOOL_DURATION"] += intval($arFields["PROPERTY_SCHEDULE_DURATION_VALUE"]);
			}

			//$arCourse=GetIblockElement($arItem);
			//$arResult["SCHOOL_STRUCTURE"][$arCourse['ID']] = $arCourse;
		}
	}	
