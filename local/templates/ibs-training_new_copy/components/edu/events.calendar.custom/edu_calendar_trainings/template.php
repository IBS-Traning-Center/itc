...
<div class="news-calendar l" id="calendar">
				...
				<table border="0" width="100%"  cellspacing="0" cellpadding="5">
					<tr>
						...
						<?if($arParams["SHOW_CURRENT_DATE"]):?>
							<h2 style="text-align: center; margin-bottom:-4px; color:#701132;"><?=$arResult["TITLE"]?></h2>
						<?endif?></td>
						<td width="10%" height=30>
...
<?$total_number = 0;?>
	...
	<div style="" class="w445 l"> <!-- second column -->
		<div class="wrapper">

		<?if ($total_number>0) {?>
			<h2>Занятия в <?=$name_month?>  <?=$arResult["currentYear"]?>:</h2>
				<?foreach($arResult["MONTH"] as $arWeek):?>
					<?foreach($arWeek as $arDay):?>
								<?$index = 0;?>
								<?foreach($arDay["events"] as $arEvent):?>
								...
								?>
									...
									<div class="month_<?=$arDay['day']?> auto table_data" date="<?=$arDay['day']?>">
										 ...
										 <div class="l w80 training_code" >
                                           &nbsp;&nbsp;<?=$arEvent['course_code']?>
		       						     </div>
										 <div class="r w280">
											 <? if (strlen($arEvent["link"]) < 1) {?><a  class="" href="/training/catalog/course.html?ID=<?=$arEvent['schedule_course']?>"><?=$arEvent["title"]?></a><?} else {?><a class="black" href="<?=$arEvent['link']?>"><?=$arEvent["title"]?></a><? } ?>
										 </div>
		...
		<? }  else { ?>
		  <h2>Нет мероприятий (занятий) в этом месяце</h2>
		<?  } ?>
		</div>
	</div>
</div>
...
<script type="text/javascript">
	jsAjaxUtil.EvalGlobal(Activation2());
</script>
