	...
	<?foreach($arResult["ITEMS"] as $key => $arItem):
		...
		if($key <= 2) {
			...
			<div class="history__list__item<?=($key == 0) ? ' active' : '';?>" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
				<div class="history__list__item__head">
				...
				<div class="history__list__item__text d-lg-none" style="<?=($key === 0) ? '' : 'display: none;';?>"><?=$arItem['PREVIEW_TEXT']?></div>
			</div>
			<?
		}
	?>
	...
	<?foreach($arResult["ITEMS"] as $key => $arItem):
		...
		if($key > 2) {
			...
			<div class="history__list__item" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
				...
				<div class="history__list__item__text d-lg-none" style="display: none;"><?=$arItem['PREVIEW_TEXT']?></div>
			</div>
			<?
		}
	?>
