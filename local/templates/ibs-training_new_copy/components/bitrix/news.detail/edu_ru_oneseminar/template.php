...
<? if(!$startdate=="")  {  ?>
	<div class="st">Дата проведения:</div>
	<p class="indent" id="from_event_date"> <a class="add-to-calendar" href="/events/seminar/ics.html?ID=<?=$arResult['ID']?>" data-type="Timetable"></a> <?=$startdate?>
</p>
<? } ?>
<? if(!$time=="")  {  ?>
	<div class="st">Время:</div>
	<p class="indent"><?=$time?>
<? if ($arResult["PROPERTIES"]["type_event"]["VALUE_ENUM_ID"]==92){?>
 (мск.)
<? } ?>
...
<? if ($arResult["PROPERTIES"]["type_event"]["VALUE_ENUM_ID"]==92){?>
	<div class="floated_right w70">
		<span class="links "><a class="" href="/events/seminar/webinar_rules.html">Правила участия в вебинаре</a></span>
	</div><br />
<? } ?>
<? if((strlen($description)>0) or (strlen($arResult['PROPERTIES']['ADDITIONAL_DESC']['~VALUE']['TEXT'])>0))  {  ?>
	<div class="st">Краткое описание</div>
...
<?  //ID_PREDV_COURSES  course_format
	...
	$arIDCourses = array();
	if  (($varNumberLinkedCourses > 0) and  array_key_exists("0", $arResult["PROPERTIES"]["ID_LINKED_COURSES"]["VALUE"])){?>
	<div class="st">Связанные курсы</div>
	<div class="indent">
           <blockquote>
			<ul class="linked">
		<?
...
<? if(strlen($arResult["PROPERTIES"]["file"]["VALUE"]) > 0)  {  ?>
	<div class="st">Дополнительные файлы</div>
...
<?if (($arResult["PROPERTIES"]["FLAG_CLOSE_REG"]["VALUE_ENUM_ID"] <> 101) and ($arResult["ID"] !== "28375")) {?>
	<p><div class="st">Участие в мероприятии бесплатное при условии предварительной регистрации.</div><br /></p>
<? } ?>
<? global $USER;
if ($USER->IsAdmin()) {
...
?>
    ...
    <script type="text/javascript">
        var YaShareInstance = new Ya.share({
            ...
            elementStyle: {
                        type: 'none',
                        quickServices: ['', 'yaru', 'vkontakte', 'twitter', 'odnoklassniki', 'moimir', 'lj', 'moikrug', 'evernote', 'greader']
            },
