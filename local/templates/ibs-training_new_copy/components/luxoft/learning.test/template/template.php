...
<?php else:?>
	...
	*/?>
	...
	<div class="learn-question-cloud">
		...
		<div class="learn-question-name"><?if ($arResult["TEST"]["ID"]=="40") {?><?=htmlspecialchars_decode($arResult["QUESTION"]["NAME"])?><?} else {?><?=$arResult["QUESTION"]["NAME"]?><?}?>
			<?if (strlen($arResult["QUESTION"]["DESCRIPTION"]) > 0):?>
				<br /><?=$arResult["QUESTION"]["DESCRIPTION"]?>
			<?endif?>
			<?if ($arResult["QUESTION"]["QUESTION_TYPE"] == "M"):?> (выберите все
правильные варианты ответов из предложенных)<?endif?>
		...
		<?php else:?>
			<?foreach($arResult["QUESTION"]["ANSWERS"] as $arAnswer):?>
				...
				<?if ($arResult["QUESTION"]["QUESTION_TYPE"] == "M"):?>
					<label><input id="answer<?=$arAnswer["ID"]?>" type="checkbox" name="answer[]" value="<?=$arAnswer["ID"]?>" <?if (in_array($arAnswer["ID"], $arResult["QBAR"][$arResult["NAV"]["PAGE_NUMBER"]]["RESPONSE"])):?>checked <?endif?>/>&nbsp;<?if ($arResult["TEST"]["ID"]=="40") {?><?=htmlspecialchars_decode($arAnswer["ANSWER"])?><?} else {?><?=$arAnswer["ANSWER"]?><?}?></label><br />
					<div class='clearfix'></div>
				<?elseif ($arResult["QUESTION"]["QUESTION_TYPE"] == "S"):?>
					<input style="float: left; margin-right: 6px;" id="answer<?=$arAnswer["ID"]?>" type="radio" name="answer" value="<?=$arAnswer["ID"]?>" <?if (in_array($arAnswer["ID"], $arResult["QBAR"][$arResult["NAV"]["PAGE_NUMBER"]]["RESPONSE"])):?>checked <?endif?>/>&nbsp;<label for="answer<?=$arAnswer["ID"]?>" style="width: 600px;"><?if ($arResult["TEST"]["ID"]=="40") {?><?=htmlspecialchars_decode($arAnswer["ANSWER"])?><?} else {?><?=$arAnswer["ANSWER"]?><?}?></label><br />
					<div class='clearfix'></div>
				<?endif?>
	...
	<?elseif (strlen($arResult["ERROR_MESSAGE"]) > 0):?>
		...
		<?=bitrix_sessid_post()?>
		<?if ($arResult["TEST"]["TIME_LIMIT"] > 0):?>
			<p style="font-size: 14px;">На выполнение теста отводится <?=$arResult["TEST"]["TIME_LIMIT"]?> <?=GetMessage("LEARNING_TEST_TIME_LIMIT_MIN")?></p>
		<?endif?>
		<p>Для того, чтобы приступить к выполнению теста "<?=$arResult["TEST"]["NAME"]?>", нажмите кнопку "Начать"</p>
		<input type="hidden" name="back_page" value="<?=$arResult["SAFE_REDIRECT_PAGE"]?>" />
		<input type="submit" class="main-button"  name="next" value="<?=GetMessage("LEARNING_BTN_CONTINUE")?>">
		</form>
	...
	<?else:?>
		...
		<input type="hidden" name="back_page" value="<?=$arResult["SAFE_REDIRECT_PAGE"]?>" />
		<input type="submit" class="main-button" name="next" value="<?=GetMessage("LEARNING_BTN_START")?>">
		</form>
