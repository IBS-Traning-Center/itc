...
function clearTextHTML($text, $max_length = 0) {
    $clearText = strip_tags($text);
    $nbsp = html_entity_decode("&nbsp;");
    $clearText = html_entity_decode($clearText);
    $clearText = str_replace($nbsp, " ", $clearText);
    $clearText = preg_replace('/\s+/', ' ', $clearText);

    if($max_length) {
        $clearText = TruncateText($clearText, $max_length);
    }
	...
	<form action="" method="get">
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0">
			<tbody><tr>
				...
				<td>
					<input class="search-button main-button" type="submit" value="<?echo GetMessage("CT_BSP_GO")?>" />
				</td>
			</tr>
		</tbody></table>
...
<?if(isset($arResult["REQUEST"]["ORIGINAL_QUERY"])):
	...
	<?elseif(count($arResult["SEARCH"])>0):?>
		...
		<?foreach($arResult["SEARCH"] as $arItem):?>
			...
			<pre style="display: none"><?=var_export($arItem)?></pre>
				<h4><a href="<?echo $arItem["URL"]?>"><?echo $arItem["TITLE_FORMATED"]?></a></h4>
				<div class="search-preview"><?echo clearTextHTML($arItem["BODY_FORMATED"])?></div>
				<?if(
