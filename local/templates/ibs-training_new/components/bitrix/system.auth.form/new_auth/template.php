...
<?php
// AJAX обработчик для восстановления пароля
...
// НОВЫЙ AJAX обработчик для авторизации
if ($_POST['AUTH_FORM'] === 'Y' && $_POST['TYPE'] === 'AUTH' && $_REQUEST['ajax'] === 'Y')
{
    require($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/prolog_before.php");
    $APPLICATION->RestartBuffer();
    ...
    if (empty($email) || empty($password)) {
        ...
        // Проверка CAPTCHA если есть
        if (!empty($captcha_sid) && !empty($captcha_word)) {
            include_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/classes/general/captcha.php");
            $cpt = new CCaptcha();
            if (!$cpt->CheckCodeCrypt($captcha_word, $captcha_sid)) {
                $result['MESSAGE'] = 'Неверный код с картинки';
                header('Content-Type: application/json');
                echo \Bitrix\Main\Web\Json::encode($result);
                die();
            }
        ...
        // Попытка авторизации
        $arAuthResult = $USER->Login($email, $password, $remember);

        if ($arAuthResult === true) {
            $result = array(
                'STATUS' => 'OK',
                'MESSAGE' => 'Авторизация успешна',
                'REDIRECT' => $arResult["BACKURL"] ?: '/'
            );
        } else {
            $result['MESSAGE'] = is_array($arAuthResult) ? $arAuthResult['MESSAGE'] : 'Неверный логин или пароль';
        }
    }
...
<?if ($arResult["FORM_TYPE"] == "login"):?>
    <?
    // Скрываем системные ошибки Bitrix для AJAX
    if (!isset($_REQUEST['ajax']) && $arResult['SHOW_ERRORS'] == 'Y' && $arResult['ERROR']) {
                ...
                <form class="auth-form" id="authFormMain">
                    ...
                    <div class="field">
                        <input type="email"
                               ...
                               maxlength="80"
                               value="<?=htmlspecialcharsbx($arResult["USER_LOGIN"])?>"
                               required />
