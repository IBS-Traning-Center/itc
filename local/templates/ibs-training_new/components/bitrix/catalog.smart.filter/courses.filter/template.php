			...
			<div class="row">
				...
				foreach($arResult["ITEMS"] as $key=>$arItem)
				{
					if(
						empty($arItem["VALUES"]) ||
						isset($arItem["PRICE"]) ||
                        $arItem['CODE'] == 'FREE_COURSE' ||
                        $arItem['CODE'] == 'COURSE_WITH_DISCOUT'
					)
						continue;
					...
					<div class="<?if ($arParams["FILTER_VIEW_MODE"] == "HORIZONTAL"):?>col-sm-6 col-md-4<?else:?>col-lg-12<?endif?> bx-filter-parameters-box <?if ($arItem["DISPLAY_EXPANDED"]== "Y"):?>bx-active<?endif?>">
						...
						<div class="bx-filter-block" data-role="bx_filter_block">
							<div class="row bx-filter-parameters-box-container">
                                <?php if ($arItem['CODE'] == 'course_price') : ?>
                                    <?php if (!empty($arResult['FREE_COURSE'])) : ?>
                                        <div class="col-xs-12 dop-filter-value">
                                            <?foreach($arResult['FREE_COURSE']['VALUES'] as $val => $ar):?>
                                                <div class="checkbox">
                                    ...
                                    <?php endif; ?>
                                    <?php if (!empty($arResult['COURSE_WITH_DISCOUNT'])) : ?>
                                        <div class="col-xs-12 dop-filter-value">
                                            <?foreach($arResult['COURSE_WITH_DISCOUNT']['VALUES'] as $val => $ar):?>
                                                <div class="checkbox">
                                                    <label data-role="label_<?=$ar["CONTROL_ID"]?>" class="bx-filter-param-label <? echo $ar["DISABLED"] ? 'disabled': '' ?>" for="<? echo $ar["CONTROL_ID"] ?>">
                                                        <span class="bx-filter-input-checkbox">
                                                            <input
							...
							switch ($arItem["DISPLAY_TYPE"])
							{
								case SectionPropertyTable::NUMBERS_WITH_SLIDER://NUMBERS_WITH_SLIDER
                                    ...
                                    if ($arItem['CODE'] == 'course_duration') {
                                        $filterSymbol = 'ч';
                                        $dopClass = 'duration';
                                    }
                                ...
                                ?>
									<div class="col-xs-6 bx-filter-parameters-box-container-block bx-left min-value-block <?= $dopClass ?>">
										<div class="bx-filter-input-container">
											...
											/>
                                            <span class="f-16"><?= $filterSymbol ?></span>
										</div>
									</div>
                                    <span class="f-16 separate-block">-</span>
									<div class="col-xs-6 bx-filter-parameters-box-container-block bx-right max-value-block <?= $dopClass ?>">
										<div class="bx-filter-input-container">
                                            <span class="f-16">до</span>
											<input
												class="max-price"
												type="text"
												name="<?echo $arItem["VALUES"]["MAX"]["CONTROL_NAME"]?>"
