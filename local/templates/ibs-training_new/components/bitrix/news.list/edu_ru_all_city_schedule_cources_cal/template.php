...
foreach($arResult["ITEMS"] as $arItem) {
    ...
    while ($ar_fields = $res->GetNext()) {
        ...
        // Получаем значение свойства FORMAT
        $course_format = $ar_fields['PROPERTY_COURSE_FORMAT_VALUE'];
    }
    if ($schedule_price == "") {
    ...
    if (!empty($course_format)) {
        if (is_array($course_format)) {
            if (in_array('Self_Study', $course_format)) {
                $show_cart_button = true;
            }
        } else {
            if ($course_format == 'Self_Study') {
                $show_cart_button = true;
            }
        }
    }

    if ($course_certified == 'Да' || $course_certified == 'Y' || $course_certified == 'Yes') {
        $show_cart_button = true;
    }
    $show_button_flag = $show_cart_button ? "Y" : "N";
    ...
    $arValueOfCourses[$ii]["show_cart_button"] = $show_button_flag;
    ...
    <div class="price">
        ...
        <div class="price-wrapper" style="justify-content:space-between;">
            ...
            <div class="cart-btn-cont">
                <?php if ($value["show_cart_button"] == "Y" && $value["show_basket"] == "Y"): ?>
                    <button
                            class="btn-add-to-cart add-to-cart-btn"
                            data-course-id="<?= $value["course_id"] ?>"
                            data-course-name="<?= htmlspecialcharsbx($value["name"]) ?>"
                            data-course-price="<?= $value["price"] ?? 0 ?>"
                            data-schedule-id="<?= $value['schedule_id'] ?>"
                            data-schedule-name="<?= htmlspecialcharsbx($value['name'] ?? '') ?>"
                            data-schedule-start="<?= $value['startdate'] ?? '' ?>"
                    >
                        В корзину
                    </button>
                <?php endif; ?>
