        ...
        <div class="auth-container">
            ...
            <div class="tabs">
                ...
                <? if($arResult['NEW_USER_REGISTRATION'] == 'Y'): ?>
                    <button class="tabs__item" onclick="window.location.href='/auth/registration.php'">
                        Регистрация
                    </button>
                <? else: ?>
            ...
            <div id="at_frm_bitrix">
                <form class="auth-form" id="authFormMain" method="post" target="_top" action="<?=$arResult["AUTH_URL"]?>">
                    ...
                    <div class="field">
                        <input type="email" name="USER_LOGIN" placeholder="Эл. почта" id="authEmailInput"
                               maxlength="80" value="<?=htmlspecialcharsbx($arResult["USER_LOGIN"])?>" required />
                        <div class="field-error" id="authEmailError"></div>
                    </div>

                    <div class="field field--password">
                        <input type="password" name="USER_PASSWORD" placeholder="Пароль" id="authPasswordInput" maxlength="50" required />
                        <span class="field__icon"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M11.9984 9.00462C14.2075 9.00462 15.9984 10.7955 15.9984 13.0046C15.9984 15.2138 14.2075 17.0046 11.9984 17.0046C9.78927 17.0046 7.99841 15.2138 7.99841 13.0046C7.99841 10.7955 9.78927 9.00462 11.9984 9.00462ZM11.9984 10.5046C10.6177 10.5046 9.49841 11.6239 9.49841 13.0046C9.49841 14.3853 10.6177 15.5046 11.9984 15.5046C13.3791 15.5046 14.4984 14.3853 14.4984 13.0046C14.4984 11.6239 13.3791 10.5046 11.9984 10.5046ZM11.9984 5.5C16.6119 5.5 20.5945 8.65001 21.6995 13.0644C21.8001 13.4662 21.5559 13.8735 21.1541 13.9741C20.7523 14.0746 20.345 13.8305 20.2444 13.4286C19.3055 9.67796 15.9198 7 11.9984 7C8.07534 7 4.68851 9.68026 3.75127 13.4332C3.6509 13.835 3.24376 14.0794 2.84189 13.9791C2.44002 13.8787 2.1956 13.4716 2.29596 13.0697C3.39905 8.65272 7.38289 5.5 11.9984 5.5Z" fill="#212121"/>
                        ...
                        <div class="field">
                            <div style="margin-bottom: 16px;">
                                <img src="/bitrix/tools/captcha.php?captcha_sid=<?=$arResult["CAPTCHA_CODE"]?>"
                                     width="180" height="40" alt="CAPTCHA" id="captchaImg" />
                                <input type="hidden" name="captcha_sid" value="<?=$arResult["CAPTCHA_CODE"]?>" />
                            </div>
                            <input type="text" name="captcha_word" placeholder="<?=GetMessage("AUTH_CAPTCHA_PROMT")?>"
                                   maxlength="50" value="" id="captchaInput" required />
                            <div class="field-error" id="captchaError"></div>
                        </div>
                    <? endif; ?>

                    <div class="auth-result" id="authResult" style="display: none;"></div>
                    ...
                    <button type="submit" name="Login" class="auth-form__submit" id="authSubmitBtn">
        ...
        <div class="auth-overlay" id="authOverlay" style="display:none;">
            <div class="auth-modal auth-modal--reset" id="resetPasswordModal">
                <button class="auth-modal__close" id="closeResetModal"></button>

                <div class="auth-modal__content">
                    <h2 class="auth-modal__title">Восстановление пароля</h2>
                    ...
                    <form id="resetPasswordForm">
                        ...
                        <div class="auth-modal__buttons">
                            <button type="button" class="auth-modal__button auth-modal__button--outline" id="backToAuthBtn">
                                Назад
                            </button>

                            <button type="submit" class="auth-modal__button auth-modal__button--submit" id="submitResetForm">
                                Отправить
                            </button>
                        </div>

                        <div id="resetResultMessage" class="auth-modal__result"></div>
                    </form>
                </div>
            </div>
        </div>
        <div class="auth-overlay" id="successOverlay" style="display:none;">
            <div class="auth-modal auth-modal--success" id="successPasswordModal">
                <button class="auth-modal__close" id="closeSuccessModal"></button>

                <div class="auth-modal__content">
                    <h2 class="auth-modal__title">Пароль отправлен</h2>

                    <div class="auth-modal__message" id="successMessage">
                        На почту <span class="auth-modal__email" id="successEmailText"></span> отправлен новый пароль<br>
                        Если письма нет — проверьте папку Спам
                    </div>

                    <div class="auth-modal__buttons auth-modal__buttons--single">
                        <button type="button" class="auth-modal__button auth-modal__button--full" id="returnToAuthBtn">
                            Вернуться ко входу
                        </button>
                    </div>
                </div>
