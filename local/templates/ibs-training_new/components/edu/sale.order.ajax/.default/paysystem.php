...
<table width="100%" cellpadding="6" cellspacing="0">
	<?
	$isPTRN="N";
    foreach ($arResult["BASKET_ITEMS"] as $arBaskItem) {
        if ($arBaskItem["CAN_BUY"]=="Y") {
            foreach($arBaskItem["PROPS"] as $arProp) {

               if ($arProp["CODE"]=="COURSE_CODE") {
                    if (preg_match("#PTRN#", $arProp["VALUE"])){
                        $isPTRN="Y";
                    }
               }
            }
	...
	if ($arResult["PAY_FROM_ACCOUNT"]=="Y" && $isPTRN=="N")
	{
		?>
		<tr>
		<td colspan="2">
	...
	<tr>
		<td colspan="2" style="padding-top: 4px;">
			<b>Если вы находитесь за пределами Российской Федерации, свяжитесь, пожалуйста,  с нашим менеджером для получения счета на оплату. Отправьте ваш запрос на <a href="mailto: <?=EMAIL_ADDRESS?>"><?=EMAIL_ADDRESS?></a>. </b>
		</td>
	</tr>
</table>
</div>
