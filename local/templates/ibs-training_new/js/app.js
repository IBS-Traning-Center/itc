document.addEventListener('click', function (e) {
    // Проверяем несколько возможных классов иконки
    const icon = e.target.closest('.eye-icon, .field__icon');

    if (icon) {
        console.log('Клик по иконке пойман!');
        console.log('Элемент:', icon);
        console.log('Классы иконки:', icon.className);

        // Пробуем найти input самым надёжным способом
        const input = icon.previousElementSibling;

        if (input && (input.tagName === 'INPUT')) {
            console.log('Input найден:', input);
            console.log('Текущий type:', input.type);

            // Меняем тип
            input.type = input.type === 'password' ? 'text' : 'password';

            // Меняем иконку (просто текст для теста)
            ...
            console.error('Input НЕ найден рядом с иконкой');
            // Альтернативные способы поиска input
            console.log('Пробуем найти input через родительский контейнер...');

            // Вариант 1: Поиск внутри общего родителя
