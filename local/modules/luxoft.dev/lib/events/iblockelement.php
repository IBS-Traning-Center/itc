...
class IBlockElement
{
    ...
    public static function OnAfterIBlockElementAddLast (&$arFields) {
        ...
        if ($arFields["IBLOCK_ID"] == 6) {
            ...
            while ($ob = $res->GetNextElement()) {
                ...
                if ((strtoupper($ar_fields["PROPERTY_COURSE_CODE_VALUE"]) !== $arFields["CODE"])
                    and (strlen($ar_fields["PROPERTY_COURSE_CODE_VALUE"]) > 0)) {
                    $el = new CIBlockElement;
                    $arCode = array(
                        "CODE" => strtoupper($ar_fields["PROPERTY_COURSE_CODE_VALUE"]),
                    );
                    $result = $el->Update($arFields["ID"], $arCode);
                }
                if ((($ar_fields["XML_ID"]) != strtolower(codeTranslite($ar_fields['NAME'])))
                    and (strlen($ar_fields["XML_ID"]) > 0)) {
                    $el = new CIBlockElement;
                    $arCode = array(
                        "XML_ID" => strtolower(codeTranslite($ar_fields['NAME'])),
...
Более подробно о сертификации Вы можете узнать у менеджера отдела продаж.';
                ...
                */
                if (($arFields3["PROPERTY_TYPE_ENUM_ID"] == 78) and
                    ...
                    ($pos === false)) {
                    if (
                        preg_match("#luxoft#", strtolower($arFields["PROPERTY_VALUES"][245])) ||
                ...
                */
                if ($arFields3["PROPERTY_TYPE_ENUM_ID"] == 205) {
                    ...
                    if (preg_match("#luxoft#", $arSendEvent["EDU_EVENT_USER_EMAIL"])) // сотрудник Luxoft
                    {
    ...
    public static function OnAfterIBlockElementAdd(& $arFields) {
        switch ($arFields['IBLOCK_ID']) {
            case 171:
                \Luxoft\Dev\Tools\Mail::sendMailEventRegistration($arFields);
                \Luxoft\Dev\Tools\Subscribe::addEuroNews($arFields);
                break;
        }
    }
