...
$arResult = array(
    ...
    "bUsingVat" => false,
    "BASKET_ITEMS" => array(),
    "BASE_LANG_CURRENCY" =>  ($_POST['PERSON_TYPE'] == 9) ? 'GRN' : CSaleLang::GetLangCurrency(SITE_ID),
...
{
    if ($_POST["checker"] != "iamnotbot") {
        $arResult["ERROR"][]="CAPTCH_ERROR";
    }
    if(strlen($_REQUEST["ORDER_ID"]) <= 0)
    {
        ...
        {
            if($_POST['PERSON_TYPE'] == 9) {
                $db_res = CPrice::GetList(array(), array("PRODUCT_ID" => $arItem['PRODUCT_ID'], "CATALOG_GROUP_ID" => 3));
        ...
        if($arUserResult["CONFIRM_ORDER"] == "Y" && empty($arResult["ERROR"]))
        {
            if(!$USER->IsAuthorized() && $arParams["ALLOW_AUTO_REGISTER"] == "Y")
            {
                if(strlen($arUserResult["USER_EMAIL"]) > 0)
                {
                    ...
                    {
                        $arResult["ERROR"][] = GetMessage("STOF_ERROR_REG").((strlen($user->LAST_ERROR) > 0) ? ": ".$user->LAST_ERROR : "" )." Авторизуйтесь на сайте или введите другой e-mail.";
                    }
                    else
                    {
            ...
            if($USER->IsAuthorized() && empty($arResult["ERROR"]))
            {
                ...
                if (empty($arResult["ERROR"]))
                {
                    ...
                    while ($arProps = $order_props->Fetch())
                    {
                        ...
                        {
                            $onlyname = $arProps["VALUE"];
                        }
                        if ($arProps["CODE"] == "F_S_NAME")
                        {
                            $sename = $arProps["VALUE"];
                        }
                    ...
                    $arFields["FULL_NAME"]= $arFields["ORDER_USER"]." ".$onlyname." ".$sename;
