...
if (
    !isset($arParams["COLUMNS_LIST"])
    || !is_array($arParams["COLUMNS_LIST"])
    || count($arParams["COLUMNS_LIST"]) <= 0
) {
...
)
{
	if(strlen($_REQUEST["action"]) > 0)
	{
		...
		if($id > 0 && $arBasket = CSaleBasket::GetByID($id))
		{
...
if ($arDiscount = $dbDiscount->Fetch())
{
	...
	else
	{
		...
		for ($bi = 0; $bi < count($arResult["ITEMS"]["AnDelCanBuy"]); $bi++)
		{
			$curDiscount = roundEx(DoubleVal($arResult["ITEMS"]["AnDelCanBuy"][$bi]["PRICE"]) * $arResult["DISCOUNT_PRICE"] / $allSum, SALE_VALUE_PRECISION);
