...
class ChangePrice20230112151610 extends Version
{
    ...
    public function up()
    {
        ...
        if (!isset($this->params['result'])) {
            ...
            $result = Json::decode($this->params['result']);
        }
        ...
        foreach ($priceElements as $element) {
            ...
            $element->set('PRICE', $coursePrices[$courseCode]);
        ...
        if ($this->params['count'] < count($AllCoursePrices)) {
            $this->params['result'] = Json::encode($result);
            $this->restart();
        }

        foreach ($result as $courseCode => $resultValue) {
            if ($resultValue['status'] == 'error') {
                $this->outError($courseCode . ' - ' . $resultValue['message']);
            } else {
                $this->out($courseCode . ' - ' . $resultValue['message']);
            }
        }
    }
