...
?>
<?php $APPLICATION->IncludeComponent("bitrix:form.result.new", "send_price_list_2", array(
    ...
    <section class="bg-main-wrap"
             style="background: url('/static/images/bg-catalog.jpg') center 0; background-size: cover;">
        <div class="frame">
            ...
            <?php $APPLICATION->IncludeComponent("bitrix:menu", "right-menu-more", array(
            ...
            <div class="search-item-catalog">
                <form id="filter">
                    ...
                    <?php foreach ($arCat as $catergory) { ?>
                        <input style="display: none;" type="checkbox" class="no_redraw"
                               <?php if (in_array($catergory["ID"], $_REQUEST["cat"] ?? [])) { ?>checked="checked"<?php } ?>
                               value="<?= $catergory["ID"] ?>" name="cat[]"/>
                    <?php } ?>
                </form>
            </div>
            <?php if (is_array($arSelected)) { ?>
                <ul class="selected-items">
                    <?php foreach ($arSelected as $secCat) { ?>
                        <li><?= $secCat["NAME"] ?> <a class="delete-cat" data-id="<?= $secCat["ID"] ?>"
            ...
            <div class="timetable-filter-wrap">
                <?php if ($_REQUEST["type"] != "online") { ?>
                    Раcписание курсов в <?php include $_SERVER["DOCUMENT_ROOT"] . SITE_TEMPLATE_PATH . '/include/city-select.php'; ?>
                <?php } ?>
                <div <?php if ($_REQUEST["type"] == "online") { ?> style="margin-left: 0;"<?php } ?>
                    class="simple-select category-picker"><a class="title dropdown-link" href="">Выберите направление <i
                            class="fa fa-caret-down" aria-hidden="true"></i></a>
                    <ul class="dropdown">
                        <?php foreach ($arCat as $catergory) { ?>
                            <li><a data-id="<?= $catergory["ID"] ?>"
                                   href="javascript:void(0)"><?= $catergory["NAME"] ?></a></li>
                        <?php } ?>
                    </ul>
                </div>
                <?php /*<div class="simple-select"><a class="title dropdown-link" href="">Выберите направление <i class="fa fa-caret-down" aria-hidden="true"></i></a><ul class="dropdown"><li><a href="#">Тестирование</a></li><li><a href="#">Разработка ПО</a></li><li><a href="#">Методологии разработка ПО</a></li><li><a href="#">Системный и бизнес анализ</a></li><li><a href="#">Коммуникации</a></li><li><a href="#">Финансовые рынки</a></li></ul></div>*/ ?>
            </div>
...
if ($_REQUEST["cat"][0] == '83005' || $_REQUEST["cat"][0] == '5723') {
    ...
    <section id="banner" class="banner-main-page"><?php $APPLICATION->IncludeComponent(
...
if (($id_type == 2) or ($id_type == 4)) {
    if (isset($_GET["by_date"])) { ?>
        <section id="content" class="bg not-main-page">
            <?php $APPLICATION->IncludeComponent("edu:events.calendar.custom", "edu_calendar_trainings_by_cat", array(
                    "IBLOCK_TYPE" => "edu",
            ...
            <div class="timetable-menu ">
                <div class="frame no-y-padding clearfix">
                    ...
                    <div class="sorting-wrap">
                        <span>Сортировка по: </span> <a
                            <?php if ($_REQUEST["sort"] != "direction") { ?>class="active"<?php
                        } ?> href="<?= $APPLICATION->GetCurPageParam("sort=date", array("sort")); ?>">дате</a> | <a
                            <?php if ($_REQUEST["sort"] == "direction") { ?>class="active"<?php
                        } ?>
                            href="<?= $APPLICATION->GetCurPageParam("sort=direction", array("sort")); ?>">направлению</a>
                    </div>
                </div>
            </div>
...
if ($id_type == 3) {
    if (!isset($_GET["by_date"])) {
        ...
        $GLOBALS["arrFilterS"] = [
            "ACTIVE" => "Y",
            ">=PROPERTY_startdate" => ConvertDateTime(date("d.m.Y H:i:s"), "YYYY-MM-DD HH:MM:SS")
        ];
        if (strlen($_REQUEST["qcat"]) > 0) {
