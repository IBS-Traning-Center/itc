    ...
    <section class="bg-main-wrap" style="background: url('/static/images/bg-catalog.jpg') center 0; background-size: cover;">
        <div class="frame">
            ...
            <div class="clearfix heading-white">
                ...
                <div class="form-wrapper" id="sendPriceForm" style="display:none;position: absolute;background: white;z-index: 999;top: 5%;left: auto;right: 5%;border-radius: 15px;border: solid 2px gray;padding: 15px;">
                    <form name="send_price_from" method="POST" enctype="application/json">
                        ...
                        <div class="table-responsive">
                            <table class="form-table data-table">
                                ...
                                <tr>
                                    ...
                                    <td><input type="text" id="sendPriceNameField" class="inputtext" name="form_text_753" value="<? if ($USER->IsAuthorized()) {
                                ...
                                <tr>
                                    ...
                                    <td><input type="text" id="sendPriceNameEmailField" class="inputtext" name="form_email_755" value="<? if ($USER->IsAuthorized()) {
                                            echo($USER->GetEmail());
                                        } ?>" size="60"></td>
                                </tr>
                                </tbody>
                                ...
                                <tr>
                                    <th colspan="2">
                                        <a onclick="sendPriceSubmit(); return false;" style="display: inline-block;margin: 15px auto 0 auto;outline: none;background-color: #225267;text-transform: uppercase;font-family: 'Open Sans', sans-serif;font-weight: bold;font-size: 14px;padding: 10px 20px;border-radius: 20px;-webkit-appearance: button;cursor: pointer;color: white;border: 1px solid transparent;">Отправить</a>
                                    </th>
                                </tr>
                                </tfoot>
                            </table>
                        ...
                        <p>
                            <span class="form-required starrequired" style="color: black;">*- обязательные поля</span>
                        </p>
                    </form>
                    <script type="text/javascript" charset="UTF-8">
                        function closeSendPriceForm() {
                            $('#sendPriceForm').hide();
                        }
                        function sendPriceSubmit() {
                            $('#sendPriceNameField').css('border-color', '');
                            $('#sendPriceNameEmailField').css('border-color', '');
                            var name = $('#sendPriceNameField').val();
                            var email = $('#sendPriceNameEmailField').val();
                            if ($.trim(name).length == 0 ){
                                $('#sendPriceNameField').focus();
                                $('#sendPriceNameField').css('border-color', 'red');
                                return false;
                            }
                            if($.trim(email).length == 0 || !isEmail(email)) {
                                $('#sendPriceNameEmailField').focus();
                                $('#sendPriceNameEmailField').css('border-color', 'red');
                                return false;
                            }
                            jQuery.ajax({
                                type: "POST",
                                url: 'send_price.php',
                                dataType: 'json',
                                data: {functionname: 'sendMail', arguments: ['<?=$file?>', encodeURIComponent(name), email] },
