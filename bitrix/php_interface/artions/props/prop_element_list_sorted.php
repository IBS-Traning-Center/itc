...
include_once(GetLangFileName(dirname(__FILE__)."/lang/", "/prop_element_list_sorted.php"));
/**
 * Языковой файл.
...
class CIBlockPropertyElementListSorted extends CIBlockPropertyElementList
{
    ...
    public static function GetUserTypeDescription()
    {
        return array(
            "PROPERTY_TYPE"         => "E",
            "USER_TYPE"             => "EListSorted",
            "DESCRIPTION"           => GetMessage("IBLOCK_PROP_ELISTSORTED_DESC"),
            "GetPropertyFieldHtml"  => array("CIBlockPropertyElementListSorted","GetPropertyFieldHtml"),
            "GetPropertyFieldHtmlMulty" => array('CIBlockPropertyElementListSorted','GetPropertyFieldHtmlMulty'),
            "GetAdminFilterHTML" => array('CIBlockPropertyElementListSorted','GetAdminFilterHTML'),
            "GetSettingsHTML" => array('CIBlockPropertyElementListSorted','GetSettingsHTML'),
            "PrepareSettings" => array('CIBlockPropertyElementListSorted','PrepareSettings'),
    ...
    {
        $settings = CIBlockPropertyElementListSorted::PrepareSettings($arProperty);
    ...
    function GetPropertyFieldHtml($arProperty, $value, $strHTMLControlName)
    {
        ...
        $bWasSelect = false;
        $options = CIBlockPropertyElementListSorted::GetOptionsHtml($arProperty, array($value["VALUE"]), $bWasSelect);
    ...
    function GetPropertyFieldHtmlMulty($arProperty, $value, $strHTMLControlName)
    {
        ...
        {
            $bWasSelect = false;
            $options = CIBlockPropertyElementListSorted::GetOptionsHtml($arProperty, $values, $bWasSelect);
        ...
        else
        {
            ...
            foreach($values as $property_value_id=>$value)
            {
                ...
                $bWasSelect = false;
                $options = CIBlockPropertyElementListSorted::GetOptionsHtml($arProperty, array($value), $bWasSelect);
    ...
    function GetOptionsHtml($arProperty, $values, &$bWasSelect)
    {
        ...
        if($settings["group"] === "Y")
        {
            $arElements = CIBlockPropertyElementListSorted::GetElements($arProperty["LINK_IBLOCK_ID"]);
            $arTree = CIBlockPropertyElementListSorted::GetSections($arProperty["LINK_IBLOCK_ID"]);
        ...
        {
            foreach(CIBlockPropertyElementListSorted::GetElements($arProperty["LINK_IBLOCK_ID"]) as $arItem)
            {
                $options .= '<option value="'.$arItem["ID"].'"';
                if(in_array($arItem["~ID"], $values))
                {
...
AddEventHandler("iblock", "OnIBlockPropertyBuildList", Array("CIBlockPropertyElementListSorted", "GetUserTypeDescription"));
?>