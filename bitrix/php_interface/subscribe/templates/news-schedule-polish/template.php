...
$arFilter = Array("IBLOCK_ID"=> 99, "ACTIVE" => "Y", "PROPERTY_CITY_ID"=> array(36978, 55979, 83635, 37594), '>PROPERTY_STARTDATE'=> date('Y-m-d'));
$res = CIBlockElement::GetList(Array("PROPERTY_CITY_ID"=> "ASC", "PROPERTY_STARTDATE"=>"ASC"), $arFilter, false, false, $arSelect);
$onlineCourses = array();
while($arItem = $res->GetNext())
{
    if($arItem['PROPERTY_CITY_ID_VALUE'] == 37594) {
        $onlineCourses[] = $arItem;
    } else {
        $arResult["COURSES"][] = $arItem;
    }
    unset($arItem);
}
$arResult["COURSES"] = array_merge($arResult["COURSES"],$onlineCourses);
unset($onlineCourses);
/*
  [ID] => 38069
            [~ID] => 38069
                      ...
                      <td style="padding: 0 15px;" class="emailContainer" valign="top">
                        <table  border="0" cellpadding="0" cellspacing="0" style="width: 100%">
                            <tr>
                                ...
                                <td style="background: #fff; text-align: right; font-size: 16px; color: #426192; font-weight: bold; padding-right: 28px; font-family: tahoma, geneva, sans-serif;"><img src="https://www.luxoft-training.com/images/digest2018/icon-location.jpg"/>    <?=$country?>, <?=$arCourse["PROPERTY_CITY_ID_NAME"]?></td>
                            </tr>
                        </table>
                      </td>
                    </tr>
                                        ...
                                        <table border="0" cellpadding="0" cellspacing="0" style="width: 100%">
                                            <tr>
                                                ...
                                                <td style="font-size: 14px; color: #426192; font-family: tahoma, geneva, sans-serif;"><?if (strlen($arCourse["PROPERTY_ENDDATE_VALUE"])>0) {?>
                                                    <?=date("j", strtotime($arCourse["PROPERTY_STARTDATE_VALUE"]))?>-<?=date("j M", strtotime($arCourse["PROPERTY_ENDDATE_VALUE"]))?>
                                                <?} else {?>
