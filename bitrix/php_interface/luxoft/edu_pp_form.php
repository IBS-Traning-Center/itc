  ...
  jQuery(document).ready(function(){
    //$("#content_container_ver form").validate();
         ...
         <script language="JavaScript">
           jQuery(document).ready(function() {
    		jQuery("td#tab_cont_edit5").hide();
    		jQuery("td#tab_cont_edit3").hide();
    		jQuery("td#tab_cont_edit6").hide();

			toggleSelect = function() {
		    	var action=jQuery('td#goroda select option:selected').attr("value");
                jQuery("table.phpmakerlist2 tr").hide();
                jQuery("tr.t"+action).show();
		        return false;
	         }
	        toggleSelect();

   			 jQuery("td#goroda select").change(
		      function () {
		        var action=jQuery('td#goroda select option:selected').attr("value");
                jQuery("table.phpmakerlist2 tr").hide();
                jQuery("tr.t"+action).show();
...
?>
	<tr style="";>
		<td></td>
		<td>Внимание: в поле "Название"  введите название класса как оно будет выглядеть в Расписании. Пример:"Класс инженера по автоматизированному тестированию. HP Merqury QTP". Если вы оставите это поле пустым, то в публичной части будет выведено  имя класса, выбранного  из выпадающего списка</td>
	</tr>
...
if(count($PROP)>0):
	...
	foreach($PROP as $prop_code=>$prop_fields):
        ...
        // выводим список категорий ПП - по умлочанию выведутся все включая вложенные категориии типа ядра
		if($prop_fields["ID"]=="60") {  ?>
		<?  $id_selected_value ="0";
			@$ELEMENT_ID = $_GET["ID"];
		    @$id_selected_value = $PROP['prschedule_program']["VALUE"]["$ELEMENT_ID:60"];
		    //echo $prop_fields["VALUE"]["$ELEMENT_ID:$prop_fields[ID]"];
		    //iwrite($prop_fields);
		    //iwrite($id_selected_value);
		    ?>
                <tr>
				...
				'<option value=""'.($bNoValue?' selected':'').'>'.htmlspecialcharsex(($def_text ? $def_text : GetMessage("IBLOCK_ELEMENT_PROP_NA") )).'</option>';
				  ...
				  {
				  	$findme   = 'Класс';
					$pos = strpos($ar_result["NAME"], $findme);
				    if ($pos !== false) {
  ...
  function doClick(cb){
    ...
    <style type="text/css">
		...
		if ($ELEMENT_ID>0){
         ...
         <?   }
		    //получим  данные из сериализованного массива
		    ...
		    foreach ($our_arr as $v1) {
   				foreach ($v1 as $key => $value) {
    				...
    				if ($key==="PRICE" ){
		 		    ...
		 		    ?>
                <tr>
                        <td valign="top"><h2>Выберите сперва город!</h2><?echo htmlspecialcharsex($prop_fields["NAME"]);?> (инструкция: выберите галочкой те курсы, которые будет объединять данная программа подготовки. При выборе курса, у вас появится возможность изменить его стоимость и длительность если это необходимо  (при вхождении  курса в данную программу подготовку)<br /><br />По умолчанию цена и длительность, которые вы видите подгружаются следующим образом: cначала из расписания данного курса, а если в распиании курса эти поля пустые, то подгружается цена и длительность курса из  карточки курса.<br /><br />  Показаны все  курсы, которые будут в выбранном вами городе):</td>
                        <td>
							<table class="phpmakerlist2">
								<tbody>
									<th>
										<td><strong><font color = red> Длительность </font></strong></td>
										<td><strong><font color = red> &nbsp;&nbsp; Цена </font></strong></td>
									</th>
            <?
            //$data  = date("Y-m-d");
            // получаем все курсы запланированные в расписании до сегодняшней даты
			...
			while($ob = $res->GetNextElement())
				{
                  ...
                  //теперь получим значение название города, зная ID города
						$arSelect = Array("NAME");
                   ...
                   //теперь получим длительность и цену курса из карточки курса
                  ...
                  $disabled= "disabled=\"true\"";
				   ...
				   $x_prschedule_contentChk .=  "><strong>" . $pp_startdate ." ". $pp_city_name ." </strong> ".$pp_course_code." ".  $pp_name ." <a href=\"/bitrix/admin/iblock_element_edit.php?ID=".$pp_id."&type=edu&lang=ru&IBLOCK_ID=9&find_section_section=0\" title='Ссылка на расписание курса в админке'>&rarr;</a>";
				   $x_prschedule_contentChk .= "</strong></td><td class=list2><nobr><input type=\"text\" ".$disabled." size=\"7\" name=\"duration_$pp_id\" id=\"d$pp_id\"  value=\"" . htmlspecialchars($pp_duration) . "\"> ч.";
				   $x_prschedule_contentChk .= "</nobr></td><td width=70 class=list2><nobr><input type=\"text\" ".$disabled."  size=\"7\" name=\"price_$pp_id\" id=\"p$pp_id\"   value=\"" .htmlspecialchars($pp_price) . "\"> р.</nobr></td>";
