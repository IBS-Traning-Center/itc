...
if (CModule::IncludeModule("iblock"))
{
	...
	if (strlen($_REQUEST["seo"])>0) {
        $arResult["DISCOUNT"]="Y";
    }
	if (intval($_REQUEST["ID_TIME"])>0) {
        $arResult["ID_TIME"]=intval($_REQUEST["ID_TIME"]);
    }
	if (intval($_REQUEST["CODE"])>0) {
		$arParams["ID"] = intval($_REQUEST["CODE"]);
	} else {
		$arParams["ID"] = intval($COURSE_ID);
	}
...
//echo "iblock_submit_".$arParams['ANCHOR_PARAMETER'];
//echo "<pre>";
//AddMessage2Log(print_r($_REQUEST,1), "my_module_id");
//echo "</pre>";
	if ($bAllowAccess)
	{
	
		// process POST data
		if (!empty($_REQUEST["iblock_submit_".$arParams['ANCHOR_PARAMETER'].""]) )
		{
			if ($_REQUEST["checkcap"]!="secrettext123") {
			...
			foreach ($arParams["PROPERTY_CODES"]  as $i => $propertyID)
			{
				...
				setcookie("form_PROP[".$propertyID."]", $arProperties[$propertyID][0]);
			...
			if (count($arResult["ERRORS"]) == 0)
			{
                ...
                if (strlen($_REQUEST["dis"])>0 && intval($arResult["ID_TIME"])== $arUpdateValues["PROPERTY_VALUES"]["313"]) {
                    $arSelect = array("ID", "PROPERTY_LINK_DISCOUNT");
                    $arFilter = array("ID"=>intval($arResult["ID_TIME"]), "IBLOCK_ID"=>9, "ACTIVE"=> "Y");
                    $res = CIBlockElement::GetList(array(), $arFilter, false, false, $arSelect);
                    if($ob = $res->GetNext())
                    {

                        $arUpdateValues["PROPERTY_VALUES"]["562"]= $ob["PROPERTY_LINK_DISCOUNT_VALUE"];
                    }
				...
				else
				{
					...
					$sAction = "ADD";
					$arUpdateValues["PROPERTY_VALUES"][608]=intval($_REQUEST["courseID"]);
					$arUpdateValues["PROPERTY_VALUES"][290]=intval($_REQUEST["courseID"]);
					if (!$arParams["ID"] = $oElement->Add($arUpdateValues, $bWorkflowIncluded))
					{
						$arResult["ERRORS"][] = $oElement->LAST_ERROR;
					} else {
						$arResult["FORM_RESULT_ID"] = $arParams["ID"];
			...
			if (count($arResult["ERRORS"]) == 0)
			{
				...
				$sRedirectUrl .= (strpos($sRedirectUrl, "?") === false ? "?" : "&")."strIMessage".$arParams['ANCHOR_PARAMETER']."=";
				$sRedirectUrl .= urlencode($arParams["USER_MESSAGE_".$sAction]);

				if ($arResult["FORM_RESULT_ID"] > 0) {
					if (intval($_REQUEST["PROPERTY"]["313"][0])==0) {
						$sRedirectUrl .= (strpos($sRedirectUrl, "?") === false ? "?" : "&")."INCITY=Y";
					} else {
						$sRedirectUrl .= (strpos($sRedirectUrl, "?") === false ? "?" : "&")."SCHEDULE=Y";
					}
					
					$sRedirectUrl .= (strpos($sRedirectUrl, "?") === false ? "?" : "&")."FORM_RESULT_ID=";
					$sRedirectUrl .= urlencode($arResult["FORM_RESULT_ID"]);
				}
		...
		$arResult["PROPERTY_HIDDEN"] = is_array($arParams["PROPERTY_CODES_HIDDEN"]) ? $arParams["PROPERTY_CODES_HIDDEN"] : array();
		$arResult["TYPE_ID"] = $arParams["PROPERTY_TYPE_EVENT"];
        ///gdfgdgdfgfg my code
		if ($arParams["ID"] > 0)
		{
		...
		if ($bShowForm)
		{
			...
			$arResult["MESSAGE"] = htmlspecialcharsex($_REQUEST["strIMessage".$arParams['ANCHOR_PARAMETER']]);
			$arResult["MESSAGE".$arParams['ANCHOR_PARAMETER'].""] = htmlspecialcharsex($_REQUEST["strIMessage".$arParams['ANCHOR_PARAMETER']]);
			//AddMessage2Log(print_r($arResult,1), "my_module_id");
