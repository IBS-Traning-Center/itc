...
if($arParams["TYPE"] || !in_array($arParams["DATE_FIELD"], array("DATE_ACTIVE_FROM", "DATE_ACTIVE_TO", "TIMESTAMP_X", "DATE_CREATE","PROPERTY_STARTDATE")))
...
if($this->StartResultCache(false, array($currentMonth, $currentYear, $todayYear, $todayMonth, $todayDay)))
{
	...
	$arResult["TITLE"] = GetMessage("IBL_NEWS_CAL_M_".date("n", mktime(0, 0, 0, $currentMonth, 1, $currentYear)))." ".$currentYear;
   	$arResult["TITLE_NO_YEAR"] = GetMessage("IBL_NEWS_CAL_M_".date("n", mktime(0, 0, 0, $currentMonth, 1, $currentYear)));
	...
	$arFilter = Array(
			...
			//"PROPERTY_CITY" => $arParams['PROPERTY_CITYCHECK'],

		);
       //iwrite($arFilter);
		if ($arParams['PROPERTY_CITYCHECK']>0){
			$arFilter["PROPERTY_CITY"] = $arParams['PROPERTY_CITYCHECK'];     /*выборка по городу*/
         }
//            $arFilter["ACTIVE_FROM"] = date($DB->DateFormatToPHP(CSite::GetDateFormat("SHORT")), time());

	$arSelectedFields = Array("ACTIVE", $arParams["DATE_FIELD"], "ID", "IBLOCK_ID", "SITE_ID", "DETAIL_PAGE_URL","PROPERTY_CITY", "PROPERTY_TYPE", "EXTERNAL_ID", "PROPERTY_LINK", "PROPERTY_SCHEDULE_TIME", "PROPERTY_COURSE_CODE", "PROPERTY_STARTDATE", "PROPERTY_SCHEDULE_COURSE", "ACTIVE_TO", "ACTIVE_FROM", "NAME", "SORT", "IBLOCK_TYPE", "PREVIEW_TEXT", "PREVIEW_TEXT_TYPE");
//iwrite($arSelectedFields);
//iwrite($arFilter);
	$dbItems = CIBlockElement::GetList(array("PROPERTY_STARTDATE"=>"ASC", "ID"=>"ASC"), $arFilter, false, false, $arSelectedFields);
...
//iwrite($arItem);
		...
		$arDays[ConvertDateTime($arItem["PROPERTY_STARTDATE_VALUE"], CLang::GetDateFormat("SHORT"))][] = $arItem;
		...
		for ($j = 0; $j < 7; $j++)
		{
			...
			if(is_set($arDays[$tmpDate]))
			{
				...
				foreach($arDays[$tmpDate] as $dayNews)
				{
                    ...
                    //iwrite($dayNews);
                    //die;
                    /*[PROPERTY_CITY_VALUE] => 5741 [~PROPERTY_CITY_VALUE] => 5741 [PROPERTY_CITY_VALUE_ID] => 29380 [~PROPERTY_CITY_VALUE_ID] => 29380 [PROPERTY_TYPE_VALUE] => Семинары [~PROPERTY_TYPE_VALUE] => Семинары [PROPERTY_TYPE_ENUM_ID] => 85
                    */
					$arDay["events"][] = array(
						"time"=>$eTime,
						...
						"id"=> $dayNews["ID"],
						"city_id"=>$dayNews["PROPERTY_CITY_VALUE"],
                        "type_name"=>$dayNews["PROPERTY_TYPE_VALUE"],
                        "type_enum_id"=>$dayNews["PROPERTY_TYPE_ENUM_ID"],
                        "link"=>$dayNews["PROPERTY_LINK_VALUE"],
						"active_to"=>$dayNews["ACTIVE_TO"],
						"external_id"=>$dayNews["EXTERNAL_ID"],
						"active_from"=>$dayNews["PROPERTY_STARTDATE_VALUE"], 
						"time"=>$dayNews["PROPERTY_SCHEDULE_TIME_VALUE"],   						
						"course_code"=>$dayNews["PROPERTY_COURSE_CODE_VALUE"],
						"schedule_course"=>$dayNews["PROPERTY_SCHEDULE_COURSE_VALUE"],
					);
					//iwrite($arDay["events"]);
