...
if($this->StartResultCache(false, array($currentMonth, $currentYear, $todayYear, $todayMonth, $todayDay)))
{
	...
	$arFilter = Array(
		...
		if ($arParams['PROPERTY_CITYCHECK']>0){
			$arFilter = array (
				"PROPERTY_CITY" => $arParams['PROPERTY_CITYCHECK'],     /*выборка по городу*/
			);
         }


	$arSelectedFields = Array("ACTIVE", $arParams["DATE_FIELD"], "ID", "IBLOCK_ID", "SITE_ID", "DETAIL_PAGE_URL","PROPERTY_CITY","PROPERTY_SCHEDULE_COURSE", "PROPERTY_SCHEDULE_TIME", "PROPERTY_TEACHER","PROPERTY_HOT_CHECKBOX", "PROPERTY_SCHEDULE_DURATION", "PROPERTY_SCHEDULE_PRICE", "ACTIVE_TO", "ACTIVE_FROM", "NAME", "LANG_DIR", "SORT", "IBLOCK_TYPE", "PREVIEW_TEXT", "PREVIEW_TEXT_TYPE");
			...
			if(is_set($arDays[$tmpDate]))
			{
				...
				foreach($arDays[$tmpDate] as $dayNews)
				{
					...
					$arDay["events"][] = array(
						...
						"teacher_id"=>$dayNews["PROPERTY_TEACHER_VALUE"],
						"course_id"=>$dayNews["PROPERTY_SCHEDULE_COURSE_VALUE"],
						"schedule_course_id"=>$dayNews["PROPERTY_SCHEDULE_COURSE_VALUE"],
						"schedule_time"=>$dayNews["PROPERTY_SCHEDULE_TIME_VALUE"],
						"teacher_id"=>$dayNews["PROPERTY_TEACHER_VALUE"],
						"hot_checkbox"=>$dayNews["PROPERTY_HOT_CHECKBOX_VALUE"],
						"duration"=>$dayNews["PROPERTY_SCHEDULE_DURATION_VALUE"],
						"price"=>$dayNews["PROPERTY_SCHEDULE_PRICE_VALUE"],
