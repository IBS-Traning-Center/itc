...
if($this->StartResultCache(false, array($USER->GetGroups(), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter)))
{
	...
	if($arResult = $rsIBlock->GetNext())
	{
		...
		//WHERE
        $today = date("d.m.Y");
 		//$GLOBALS["arrFilter"] = array(">=PROPERTY_startdate" => ConvertDateTime($today, "YYYY-MM-DD"),  );
        if ($arParams['PROPERTY_DATECHECK'] ==="0") {$dateparam = ">=";}
        if ($arParams['PROPERTY_DATECHECK'] ==="1") {$dateparam = "<=";}
	...
	if ($arParams['PROPERTY_CITYCHECK']>0){
			$arFilter = array (
				...
				$dateparam."PROPERTY_startdate" => ConvertDateTime($today, "YYYY-MM-DD"),
			);
         } else {
 			$arFilter = array (
				"IBLOCK_ID" => $arResult["ID"],
