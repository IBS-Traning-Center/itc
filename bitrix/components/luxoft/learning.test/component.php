...
else
{
	...
	$arResult["TEST"]["NAME"]=$arTest['NAME'];
...
if ($sessAttemptFinished===true) {
	//if ($USER->IsAdmin()) {
		$arSelect = Array("ID", "NAME", "DATE_ACTIVE_FROM");
		$arFilter = Array("IBLOCK_ID"=> 6, "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y", "PROPERTY_COURSE_LEARNING"=> $arResult["TEST"]["COURSE_ID"]);
		...
		$rsUser = CUser::GetByID($USER->GetID());
			$arUser = $rsUser->Fetch();
			if ($arUser["PERSONAL_CITY"]=="Москва" || $arUser["PERSONAL_CITY"]=="москва") {
				$CITY=CITY_ID_MOSCOW;
			} elseif ($arUser["PERSONAL_CITY"]=="Санкт-Петербург" || $arUser["PERSONAL_CITY"]=="санкт-петербург" || $arUser["PERSONAL_CITY"]=="Спб" || $arUser["PERSONAL_CITY"]=="Spb" || $arUser["PERSONAL_CITY"]=="spb") {
				$CITY=CITY_ID_SPB;
			} elseif ($arUser["PERSONAL_CITY"]=="Киев" || $arUser["PERSONAL_CITY"]=="киев" || $arUser["PERSONAL_CITY"]="Kyiv") {
				$CITY=CITY_ID_KIEV;
			} elseif ($arUser["PERSONAL_CITY"]=="Омск" || $arUser["PERSONAL_CITY"]=="омск" || $arUser["PERSONAL_CITY"]=="omsk" || $arUser["PERSONAL_CITY"]=="Omsk") {
				$CITY=CITY_ID_OMSK;
			} elseif ($arUser["PERSONAL_CITY"]=="Днепропетровск" || $arUser["PERSONAL_CITY"]=="днепропетровск" || $arUser["PERSONAL_CITY"]=="Днепр" || $arUser["PERSONAL_CITY"]=="Dnipropetrovsk") {
				$CITY=CITY_ID_DNEPR;
			} elseif ($arUser["PERSONAL_CITY"]=="Одесса" || $arUser["PERSONAL_CITY"]=="Одеса" || $arUser["PERSONAL_CITY"]=="одесса" || $arUser["PERSONAL_CITY"]=="одеса") {
				$CITY=CITY_ID_ODESSA;
			}
			//print_r($arUser);
		CreateReccommend($USER->GetID(), $Course, $CITY);
		//echo "<div class='display: none'>321321312</div>";
	//}
	//echo "312";
}
if (!sizeof($errors)) {
	...
	if (isset($sessAttemptID) && intval($sessAttemptID) > 0)
	{
		...
		if ($arResult["COMPLETE_PERCENT"])
		{
			...
			if ($arResult["COMPLETE_PERCENT"]=="100") {
				GLOBAL $USER;
...
if (CModule::IncludeModule("learning"))
{
    $course = CCourse::GetByID($arParams["COURSE_ID"]);
	
	
    if ($arCourse = $course->GetNext())
    {
        //print_r($arCourse);
		$arResult["TEST"]["PICTURE"]=CFile::GetFileArray($arCourse["PREVIEW_PICTURE"]);
