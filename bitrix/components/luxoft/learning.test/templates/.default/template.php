		...
		<?php else:?>
			<?foreach($arResult["QUESTION"]["ANSWERS"] as $arAnswer):?>
				...
				<?elseif ($arResult["QUESTION"]["QUESTION_TYPE"] == "S"):?>
					<input style="float: left; margin-right: 6px;" id="answer<?=$arAnswer["ID"]?>" type="radio" name="answer" value="<?=$arAnswer["ID"]?>" <?if (in_array($arAnswer["ID"], $arResult["QBAR"][$arResult["NAV"]["PAGE_NUMBER"]]["RESPONSE"])):?>checked <?endif?>/>&nbsp;<label for="answer<?=$arAnswer["ID"]?>" style="width: 600px;"><?=$arAnswer["ANSWER"]?></label><br />
					<div class='clearfix'></div>
				<?endif?>
