...
if($this->StartResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter)))
{
	...
	if($arResult = $rsIBlock->GetNext())
	{
		...
		GLOBAL $arrFilter;
		//print_r($arrFilter);
		$arrFilter["PROPERTY_city"]=array(CITY_ID_MOSCOW, CITY_ID_KIEV, CITY_ID_SPB);
		$arrFilter[]=array("LOGIC"=>"AND", array("NAME"=>"%java%"), array("!NAME"=>"%NET%"));
		...
		$lastcount=$rsElement->SelectedRowsCount();
		...
		if ($lastcount<$arParams["NEWS_COUNT"]) {
			GLOBAL $arrFilter1;
			$arrFilter1["!NAME"]="%NET%";
			$arrFilter1["PROPERTY_schedule_course_type"]=array(5730, 34744, 5729, 5728, 5723);
			$arrFilter1[">PROPERTY_startdate"]=date("Y-m-d", strtotime("+21 day"));
			$arrFilter1["<PROPERTY_startdate"]=date("Y-m-d", strtotime("+91 day"));
			$arrFilter1["PROPERTY_city"]=array(CITY_ID_MOSCOW, CITY_ID_KIEV, CITY_ID_SPB);
			$arrFilter1["!ID"]=$arResult["ELEMENTS"];
			$rsElement = CIBlockElement::GetList($arSort, array_merge($arFilter, $arrFilter1), false, array("nTopCount"=>$arParams["NEWS_COUNT"]-$lastcount), $arSelect);
			$rsElement->SetUrlTemplates($arParams["DETAIL_URL"], "", $arParams["IBLOCK_URL"]);
			$lastcount=$lastcount+$rsElement->SelectedRowsCount();
			while($obElement = $rsElement->GetNextElement())
			{
		...
		if ($lastcount<$arParams["NEWS_COUNT"]) {
			$arrFilter2["!NAME"]="%NET%";
			$arrFilter2["!ID"]=$arResult["ELEMENTS"];
			$arrFilter2["PROPERTY_schedule_course_type"]=array(5730, 34744, 5729, 5728, 5723);
			$arrFilter2[">PROPERTY_startdate"]=date("Y-m-d", strtotime("+21 day"));
			$arrFilter2["<PROPERTY_startdate"]=date("Y-m-d", strtotime("+91 day"));
			
			$rsElement = CIBlockElement::GetList($arSort, array_merge($arFilter, $arrFilter2), false, array("nTopCount"=>$arParams["NEWS_COUNT"]-$lastcount), $arSelect);
			$rsElement->SetUrlTemplates($arParams["DETAIL_URL"], "", $arParams["IBLOCK_URL"]);
			$lastcount=$lastcount-$rsElement->SelectedRowsCount();
			while($obElement = $rsElement->GetNextElement())
			{
