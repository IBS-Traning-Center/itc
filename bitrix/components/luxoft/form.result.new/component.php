...
/*************************************************************************************************/
	...
	if (strlen($arResult["ERROR"]) <= 0)
	{
		...
		{
			if ($_REQUEST["check_bot"]!="iamnotbot") {
				return false;
			}
			...
			)
			{
				...
				if (check_bitrix_sessid())
				{
					...
					if($RESULT_ID = CFormResult::Add($arParams["WEB_FORM_ID"], $arResult["arrVALUES"]))
					{
						...
						if (strlen($_REQUEST["subscribe_ro"])>0) {
							CModule::IncludeModule("subscribe");
						...
						GLOBAL $USER;
						$orderer_email=$_REQUEST["form_email_655"];
						if (!$USER->IsAuthorized()) {
							...
							if($arUser=$rsUsers->GetNext()) {
								...
								$arAuthResult = $user->Add(Array(
									...
									"GROUP_ID" => $GROUP_ID,
									"NAME" => $_REQUEST["form_text_653"],
									"LAST_NAME" => $_REQUEST["form_text_654"],
									"ACTIVE" => "Y",
									"LID" => SITE_ID,
									 )
								);
								if (IntVal($arAuthResult) <= 0)
								{
									...
									CEvent::Send("NEW_USER_LK", SITE_ID, $arEventFields, "Y", "183");
						...
						$rsElement = CIBlockElement::GetList(array("SORT" => "ASC"), array("IBLOCK_ID" => D_EN_COURSE_ID_IBLOCK, "=CODE" =>$_REQUEST['CODE']), false, false, array( "ID", "NAME", "CODE", "DETAIL_PAGE_URL", "PROPERTY_IS_LINKED", "PROPERTY_COURSE_DURATION"));
						while($obElement = $rsElement->GetNextElement()) {
							$arElement = $obElement->GetFields();
							$COURSE_ID=$arElement["ID"];
						}
						$arSelect = Array("ID", "NAME", "PREVIEW_TEXT");
						$arFilter = Array("IBLOCK_ID"=>144, "PROPERTY_USER"=> $PROP["USER"], "ACTIVE_DATE"=>"Y",  "ACTIVE"=>"Y");
						$arFilter["PROPERTY_SCHEDULE_ID"]=intval($_REQUEST["form_hidden_660"]);
						//$arCourse=GetIblockElement($_REQUEST["tr_id"]);
						//$arSimpleCourse=GetIblockElement($arCourse["PROPERTIES"]["COURSE_ID"]["VALUE"]);
						$res = CIBlockElement::GetList(Array(), $arFilter, false, false, $arSelect);
						if ($res->SelectedRowsCount()==0) {
							$el = new CIBlockElement;
							$PROP = array();
							$PROP["USER"] = $USER_ID; 
							$PROP["COURSE_ID"]= $COURSE_ID;
							if (intval($_REQUEST["form_hidden_660"])>0) {
								$PROP["SCHEDULE_ID"] = intval($_REQUEST["form_hidden_660"]);
							}
							$arLoadProductArray = Array(
							  ...
							  "NAME"           => $PROP["USER"]."_".intval($_REQUEST["form_hidden_660"])."_".date("Y_m_d"),
							  "ACTIVE"         => "Y",            // активен
							);
