...
$APPLICATION->SetPageProperty("blue_title", "Эксперты Учебного центра Luxoft");
$APPLICATION->SetPageProperty("title", $arResult["NAME"] ." ". $expert_name." / Эксперты Учебного центра Luxoft");

?>
<div class="team_member">
	<?if($arParams["DISPLAY_PICTURE"]!="N" && is_array($arResult["DETAIL_PICTURE"])):?>
		<img class="detail_picture" style="max-width:250px; max-height:350px" border="0" src="<?=$arResult["DETAIL_PICTURE"]["SRC"]?>" width="<?=$arResult["DETAIL_PICTURE"]["WIDTH"]?>" height="<?=$arResult["DETAIL_PICTURE"]["HEIGHT"]?>" alt="<?=$arResult["DETAIL_PICTURE"]["ALT"]?>"  title="<?=$arResult["NAME"]?>" />
	<?endif?>
...
<div>
  ...
  <?if (strlen($expert_experience)>0) {?>
  	<strong>Профессиональный опыт:</strong><div class="indent">  <?=$expert_experience?><br /></div>
  <? } ?>
  <?if (strlen($expert_teacher)>0) {?>
  	<strong>Преподавательский опыт:</strong><div class="indent"> <?=$expert_teacher?><br /></div>
  <? } ?>
