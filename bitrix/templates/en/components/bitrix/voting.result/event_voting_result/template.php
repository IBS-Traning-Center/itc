...
<h2>Статистика голосований: (доступно авторизованным пользователям) </h1>
<?
//iwrite($arResult);
if ($arResult["VOTE"]["ID"]==1) {
$z = $DB->Query("SELECT  b_vote_event_answer.MESSAGE  FROM   b_vote_event_answer  WHERE   b_vote_event_answer.ANSWER_ID = 29", false, $err_mess.__LINE__);
while ($arAnswers = $z->Fetch()) :
   ?>
   <blockquote><?=$arAnswers["MESSAGE"]?></blockquote><br /><?
endwhile;
}
if ($arResult["VOTE"]["ID"]==3) {
$z = $DB->Query("SELECT  b_vote_event_answer.MESSAGE  FROM   b_vote_event_answer  WHERE   b_vote_event_answer.ANSWER_ID = 50", false, $err_mess.__LINE__);
while ($arAnswers = $z->Fetch()) :
   ?>
   <blockquote><?=$arAnswers["MESSAGE"]?></blockquote><br /><?
endwhile;
