...
?>
<p style="color: red"><?=ShowError($arResult["ERROR_MESSAGE"])?></p>
...
?>
<p><b><?=ShowNote($arResult["OK_MESSAGE"])?></b></p>
...
?>
<div class="vote-items-list vote-question-list voting-result-box">
...
?>
	<div class="vote-item-vote <?=($iCount == 1 ? "vote-item-vote-first " : "")?><?
						...
						<table class="vote-bar-table">
							<?foreach ($arQuestion["ANSWERS"] as $arAnswer):?>
								<tr>
									...
									<td><nobr><?=$arAnswer["PERCENT"]?>%</nobr></td>
			...
			<?foreach ($arQuestion["ANSWERS"] as $arAnswer):?>
				...
				<tr>
					<? $percent = round($arAnswer["BAR_PERCENT"] * 0.8); // (100% bar * 0.8) + (20% span counter) = 100% td ?>
						<td width="44%" style='text-align: right; padding-right: 2px;'>
						<?=$arAnswer["MESSAGE"]?>
						<? if (isset($arResult['GROUP_ANSWERS'][$arAnswer['ID']])) 
						{
					...
					<span class="vote-answer-counter"><nobr><?=($arAnswer["COUNTER"] > 0?'&nbsp;':'')?><?=$arAnswer["PERCENT"]?>%</nobr></span></td>
					<? if (isset($arResult['GROUP_ANSWERS'][$arAnswer['ID']])): ?>
						...
						<?foreach ($arGroupAnswers as $arGroupAnswer):?>
							...
							<tr>
								...
								<td><div class="vote-answer-bar" style="width:<?=$percent?>%;background-color:#<?=$arAnswer["COLOR"]?>"></div>
								<span class="vote-answer-counter"><nobr><?=($arGroupAnswer["COUNTER"] > 0?'&nbsp;':'')?>213<?=$arGroupAnswer["PERCENT"]?>%</nobr></span></td>
							</tr>
						<?endforeach?>
						<tr><td></td><td style='vertical-align:middle;'><div style='width:80%; height:1px; background-color:#<?=$arAnswer["COLOR"]?>;'></div></td></tr>
					<? else: ?>
...
?>
<br/>
Хотите, чтобы IT-гуру провел тренинг, нацеленный на решение задач именно вашей компании? 
Напишите нам на <a href="mailto:education@ibs.ru">education@ibs.ru</a>, и мы организуем для вас тренинг в корпоративном формате!
</div>