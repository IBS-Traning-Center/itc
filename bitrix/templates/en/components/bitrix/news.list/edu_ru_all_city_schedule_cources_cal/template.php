...
Asset::getInstance()->addJs('https://code.jquery.com/ui/1.13.3/jquery-ui.js');
?>
<script>
    $(document).ready(function() {
        ...
        $("a.tobasket").click(function() {
            ...
            */
            ...
            var url = '/ajax/add_course_to_basket.php?action=ADD2BASKET&id=' + id_record + '&quantity=1';
            if (parseFloat($(this).attr('data-sale')) > 0) {
                url += '&dprice=' + $(this).attr('data-sale');
            }
            $.getJSON(url, function(data) {
                $(".basketSmall").fadeOut("slow");
                $(".basketSmall").load("/ajax/show_basket.php?rand='+randInt+'", {
                    limit: 25
                }, function() {
...
foreach ($arResult["ITEMS"] as $arItem): ?>
    ...
    if (strlen($arItem['PROPERTIES']['course_sale']['VALUE']) > 0) {
        $schedule_course_sale = intval($arItem['PROPERTIES']['course_sale']['VALUE']);
        $schedule_course_sale = $schedule_price * (100 - $schedule_course_sale) / 100;
    }
    if ($schedule_enddate == "") {
    } else {
...
<table cellSpacing="0" cellPadding="5" border="0" class="edu">
    ...
    foreach ($arValueOfCourses as $key => $value) {
        ...
        <tr class="ewTableAltRow" itemscope itemtype="http://data-vocabulary.org/Event" onmouseover='ew_mouseover(this);' onmouseout='ew_mouseout(this);'>
            ...
            <td class="td_date">
                <p class="nobr"><?= $value["startdate"] ?></p>
            </td>
            <td class="td_time" validn="top">
                <div class="nobr tocenter "><?/*=$value["time"]*/ ?>
                    <?
                    //global $USER;
                    //if ($USER->IsAdmin()){
                    ?>
                    ...
                    <? } ?>
                    <br />
                    <div style="padding-top:4px;"><a title="Добавить курс в календарь" data-type="Timetable" data-action="AddToCalendar" data-name="<?= $value["course_code"] ?> <?= $value["name"] ?> || <?= $value['course_id'] ?> || <?= $value['schedule_id'] ?>" rel="nofollow" class="ical_img js-tracking" href="/training/catalog/ics_course.html?ID=<?= $value['schedule_id'] ?>"><img width="24" border="0" src="/downloads/images/47-ical_24_24.png"></a></div>
                </div>
            </td>
            <td class="td_duration">
                <p class="nobr"><?= $value["duration"]; ?> ч.</p>
            </td>
            <td class="td_price">
                <? if ($arParams['SHOW_PRICE'] !== "N") { ?>

                    <? if ($value["schedule_course_sale"] > 0) { ?>
                        <p class="nobr nobr__old-price" style="text-decoration: line-through;"><?= number_format($value["price"], 0, '', ' '); ?> <?= $value["valuta"] ?><?/*if ($valuta=="Рубли") {?> р.<? }else{ ?> грн. <? } */ ?></p>
                        <p class="nobr nobr__new-price"> <?= number_format($value["schedule_course_sale"], 0, '', ' '); ?><?= $value["valuta"] ?></p>
                    <? } else { ?>
                        <p class="nobr"><?= number_format($value["price"], 0, '', ' '); ?> <?= $value["valuta"] ?><?/*if ($valuta=="Рубли") {?> р.<? }else{ ?> грн. <? } */ ?></p>
                    <? } ?>
            ...
            <? //if ($USER->IsAdmin()) {echo '<pre>'; print_r($value); echo '</pre>'; }
            ?>
            <td class="td_buy">
                <? if ($arParams['SHOW_PRICE'] !== "N") { ?>
                    <? if ($value["show_basket"] === "Y") { ?>
                        <p class="tocenter">
                            <a class="tobasket js-tracking" rel="nofollow" data-type="Timetable" data-action="AddToBasket" data-sale="<?= $value['schedule_course_sale'] ?>" data-name="<?= $value["course_code"] ?> <?= $value["name"] ?> || <?= $value['course_id'] ?> || <?= $value['schedule_id'] ?>" title="Запомнить и положить в корзину услуг" id_basket="<?= $value['schedule_id'] ?>" href="#"><img src="/images_edu/diffs/basket_put.png" width="32" height="32" alt="Запомнить и положить в корзину услуг" border="0"></a> <br />
                        </p>
                    <? } ?>
                <? } ?>
