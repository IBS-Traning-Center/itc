...
?>
      <?if($arItem["SELECTED"]):?>
        <?if(($arItem["PARAMS"]["vip"]=='3') or ($arItem["PARAMS"]["vip"]=='4') or ($arItem["PARAMS"]["show_always"]=='Y') ) { ?>
         <li class="selectedvip<?if (strlen($arItem['PARAMS']['class_name'])>0) {?> <?=$arItem['PARAMS']['class_name']?><? } ?>"><a href="<?=$arItem["LINK"]?>"><?=$arItem["TEXT"]?></a></li>
			<style type="text/css">
         ...
         <?if($arItem["PARAMS"]["sub"]!=='1') { ?>
          <li class="selected <?if ($arItem['PARAMS']['show_always']):?> vip<?endif?><?if (strlen($arItem['PARAMS']['class_name'])>0) {?> <?=$arItem['PARAMS']['class_name']?><? } ?>"><a href="<?=$arItem["LINK"]?>"><?=$arItem["TEXT"]?></a></li>
          <? } ?>
          <?if($arItem["PARAMS"]["sub"]=='1') { ?>
          <li class="selected indent minus <?if ($arItem['PARAMS']['show_always']):?> vip<?endif?><?if (strlen($arItem['PARAMS']['class_name'])>0) {?> <?=$arItem['PARAMS']['class_name']?><? } ?>"> <a href="<?=$arItem["LINK"]?>"><?=$arItem["TEXT"]?></a></li>
          <? } ?>
         <? $a=0;?>
         <? } ?>
