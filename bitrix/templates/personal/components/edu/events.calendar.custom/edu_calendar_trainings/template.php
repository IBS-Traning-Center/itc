						...
						<? if ($index > 0) { ?>
							<div style="position: relative; height: 38px;">
								<div class="show-it"><? foreach ($arDay["events"] as $key => $arEvent) { ?>
										<? $firsttime = ""; ?><? $firsttime = preg_split("#-#", $arEvent["time"]) ?><? if ($key != 0) { ?><br /><? } ?><? if ($key == 0) { ?><?= $arDay["day"] ?><? } ?><span class="time"><?= $firsttime[0] ?></span><br /><a href="/training/catalog/course.html?ID=<?= $arEvent["schedule_course"]; ?>" class="text <? if (is_array($yellow) && in_array($arEvent["id"], $yellow)) { ?> yellow<? } ?> <? if (is_array($green) && in_array($arEvent["id"], $green)) { ?> green<? } ?>"><?= $arEvent["title"]; ?></a>
									<? } ?>
								</div>
							</div>
						<? } else { ?>
