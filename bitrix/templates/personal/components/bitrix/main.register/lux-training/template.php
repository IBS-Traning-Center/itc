...
<form method="post" class="register-form" action="<?=POST_FORM_ACTION_URI?>" name="regform" enctype="multipart/form-data">
<fieldset>
<p class="title-text">Регистрация нужна для оплаты курсов, онлайн тренингов и быстрой записи на различного рода мероприятия, проводимые учебным центром</p>
<?
...
<? if ($FIELD !== "---"){?>
	<?if ($FIELD=="NAME" || $FIELD=="WORK_COMPANY") {?>
		</div><div class="text-box">
	<?}?>
		<div class="row<?if ($arResult["REQUIRED_FIELDS_FLAGS"][$FIELD] == "Y"):?> req<?endif?>" <? if ($FIELD == $tempVaraible){?>style="display:none;"<? }?> >
			<label class="headers"><?=GetMessage("REGISTER_FIELD_".$FIELD)?>:</label>
			<?
	switch ($FIELD)
	{
		case "PASSWORD":
...
<div class="buttons">
								...
								<button name="register_submit_button" type="submit" value="<?=GetMessage("AUTH_REGISTER")?>">Зарегистрироваться	</button>
	</div>
