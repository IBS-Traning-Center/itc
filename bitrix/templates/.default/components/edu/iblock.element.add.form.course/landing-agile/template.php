 ...
 <form name="iblock_add_<?=$arParams['ANCHOR_PARAMETER']?>" action="<?=POST_FORM_ACTION_URI?>#<?=$arParams['ANCHOR_PARAMETER']?>" method="post" enctype="multipart/form-data" id="submit_form_<?=$arParams['ANCHOR_PARAMETER']?>" >
			   ...
			   <fieldset>
                <div class="row">
                    ...
                    <input required value="<?=$arUserInfo['LastName']?> <?=$arUserInfo['FirstName']?>" name="PROPERTY[244][0]" type="text">
                </div>
                <div class="row">
                    <div class="box">
                        <label>E-mail <span class="required">*</span></label>
                        <input required type="text" value="<?=$arUserInfo["EMAIL"]?>" name="PROPERTY[246][0]">
                    </div>
                    <div class="box">
                        <label>Телефон <span class="required">*</span></label>
                        <input required value="<?=$arUserInfo["PERSONAL_PHONE"]?>" name="PROPERTY[247][0]" type="text">
                    </div>
                </div>
                <div class="row">
                    <label>Компания <span class="required">*</span></label>
                    <input required value="<?=$arUserInfo['WORK_COMPANY']?>" name="PROPERTY[245][0]" type="text">
                </div>
                <div class="row">
                    <label>Город <span class="required">*</span></label>
                    <input required type="text" value="<?=$arUserInfo["WORK_POSITION"]?>" name="PROPERTY[249][0]">
                </div>
                <div class="row">
                    <label>Комментарий</label>
                    <textarea cols="30" rows="30" name="PROPERTY[345][0]"></textarea>
                </div>
               <?if(!empty($arParams['DISCOUNT_TIME'])){?>
                   <input value="<?=$arParams['DISCOUNT_TIME']?>" name="PROPERTY[562][0]" type="hidden">
               <?}?>
                <div class="centered">
					<label class="agree-text" style="text-align: left;"><input id="form-reg-agree" checked="checked" name="agree" value="Y" type="checkbox"/>Настоящим я подтверждаю, что я ознакомлен с <a style="color: #333; text-decoration: underline;" target="_blank" href="/terms-of-use/">Условиями использования</a>, условия мне понятны и я согласен соблюдать их.</label>
					<label class="agree-text" style="text-align: left;"><input id="form-reg-two" checked="checked" name="agree-2" value="Y" type="checkbox"/>Я ознакомлен с порядком обработки моих персональных данных согласно <a style="color: #333; text-decoration: underline;" target="_blank" href="/privacy-policy/">Политике в сфере персональных данных</a>.</label>
                    <br/>
					<a href="javascript:void(0)" class="btn submit">Зарегистрироваться</a>
					<input type="hidden" name="iblock_submit_<?=$arParams['ANCHOR_PARAMETER']?>" value=" ">
...
<form name="iblock_add_<?=$arParams['ANCHOR_PARAMETER']?>" action="<?=POST_FORM_ACTION_URI?>#<?=$arParams['ANCHOR_PARAMETER']?>" method="post" enctype="multipart/form-data" id="submit_form_<?=$arParams['ANCHOR_PARAMETER']?>" >
	...
	<input type="text" class="required" value="<?=$arUserInfo['LastName']?> <?=$arUserInfo['FirstName']?>" name="PROPERTY[244][0]" size="25">
	</div>
...
/*				<!--<tr>
		...
		<?endif?>
<?*/?>
		<?/*			<input onclick="javascript: pageTracker._trackEvent('FILL_FORM','COURSE','<?=$_REQUEST['ID']?>');" type="submit" class="but" name="iblock_submit_<?=$arParams['ANCHOR_PARAMETER']?>" value=" " />
...
<script type="text/javascript" >
	...
	$('.btn.submit').click(function() {
		if ($(this).hasClass('disabled')) {
			...
			$('form[name=iblock_add_<?=$arParams['ANCHOR_PARAMETER']?>]').submit();
		}
	});
