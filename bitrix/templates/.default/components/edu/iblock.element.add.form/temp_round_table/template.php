...
if ($arParams['PROPERTY_TYPE_EVENT'] == 78){?>
	...
	<?if (isset($_REQUEST["FORM_RESULT_ID"]) and (is_numeric($_REQUEST["FORM_RESULT_ID"]))){?>
		...
		<p>Вы сейчас будете перенаправлены на страницу для оплаты курса online<br /></p>
		Проблемы с перенаправлением? Пожалуйста, используйте <a href='/services/buy_course.html?action=BUY&id=<?=$arIDMethodPayment["TIMETABLE_ID"]?>'> прямую ссылку.</a>
		<script type="text/javascript">
			setTimeout('Redirect()',5000);
			function Redirect()
			{
			  location.href = '/services/buy_course.html?action=BUY&id=<?=$arIDMethodPayment["TIMETABLE_ID"]?>';
			}
        ...
        <p>На ваш email отправлено письмо с приглашением посетить данный курс. Вышлите, пожалуйста, все необходимые реквизиты для составления договора в ответе на данное письмо.<br /></p>
        <? } ?>
	<? } ?>
	   <!--<span class="links"><a href="/mail/form.html">Задать вопрос по тренингу / семинару</a></span>-->
<? } ?>
...
<? //iwrite($arResult["PROPERTY_LIST_FULL"]);
$arResult["PROPERTY_LIST_FULL"]["345"]["NAME"] = "Ваши вопросы спикерам круглого стола и комментарии";
?>
...
<span class="form-required starrequired">*</span>
						...
						<textarea cols="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>" rows="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"><?=$value?></textarea>
							...
							case "N":
								...
								<input type="text" <?if(in_array($propertyID, $arResult["PROPERTY_HIDDEN"])):?>style="display:none;" <?endif?>  <?if ($propertyID=="NAME"){?>  id="event_name"  value="<?=$arEventInfo['CODE']?> <?=$arEventInfo['NAME']?>" style="display:none;" <? } ?>
								 ...
								 <?if ($propertyID==313){?>  value="<?=$arEventInfo['TIMETABLE_ID']?>"<? } ?>
					...
					[<a href="<?=$arResult["ELEMENT_FILES"][$value]["SRC"]?>"><?=GetMessage("IBLOCK_FORM_FILE_DOWNLOAD")?></a>]<br />
							...
							<select <?if(in_array($propertyID, $arResult["PROPERTY_HIDDEN"])):?>style="display:none;" <?endif?>
							 ...
							 <?if ($propertyID==313) {?>
							 name="PROPERTY[<?=$propertyID?>][0]" id="date_select" >
							 <? } else { ?>
