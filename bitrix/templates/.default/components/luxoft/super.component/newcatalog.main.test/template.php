...
foreach($arResult["ITEMS"] as $arSection):?>
	...
	if (($bPreviousIsCourse === true) and ($bFlagIsCourse === false) and ($bClassPrevious !== false)){?>
		<div class="clases_wrapper" style="border-bottom:0; background:#F9F9F7;">
			<div class="r w60"><strong><?=$vTotalPrice?> р.</strong></div>
			<div class="r w40"><strong><?=$vTotalDuration?> ч.</strong></div>
			<div class="r w335"><strong class="r">Всего:&nbsp;&nbsp;</strong></div>
			<div class="clear"></div>
		</div>
	<? } ?>
	<?
	...
	<?endif?>
	<?if (!$bFlagIsCourse):?>
		<li id="cat_<?=$arSection['ID']?>" class="<?if ($arSection["DEPTH_LEVEL"] > 1){?>open<? } else {?>closed<? } ?>"><span class="<?if ($arSection["DEPTH_LEVEL"] === "1"){?>first_one <? } else {?>twoplus <? } ?><?if ($pos !== false) {?>tr_school<? } ?>"><?=$arSection["NAME"]?></span>

				<?if ($pos !== false) {?>
					<!--<a class="orange" title="см. подробную информацию о всех курсах, входящих  в раздел <?=$arSection['NAME']?>" href="/training/catalog/section.html?SECTION_ID=<?=$arSection['ID']?>">подробнее</a>-->
		...
		<li>
			<?if ($bFlagIsCourse){?>
				<div class="clases_wrapper">
					<div class="l w335"><?=$arSection['PROPERTY_PP_COURSE_CODE']?> <a title="" href="/training/catalog/course.html?ID=<?=$arSection['PROPERTY_PP_COURSE_VALUE']?>"><?=$arSection['PROPERTY_PP_COURSE_NAME']?></a> <img border="0" src="http://www.luxoft.com/images_new/bg-author-link.gif" alt=""></div>
					<div class="r w60"><?=$arSection['PARAM']['PROPERTY_COURSE_PRICE_VALUE']?> р.</div>
					<div class="r w40"><?=$arSection['PARAM']['PROPERTY_COURSE_DURATION_VALUE']?> ч.</div>
