...
#content ul#navigation li  a.first_one {
    font-size: 18px;
    font-weight: bold;
...
#content ul#navigation li  a.twoplus {
    font-size: 15px;
    font-weight: bold;
    text-decoration: none;
    border-bottom:1px dashed #22396D;
...
.double-arrow {
    float: left;
    margin-left: 6px;
    background: url(/images_new/bgr_arr_1.gif)   0 7px repeat-x;
    height: 20px;
    width: 18px;
}
ul#navigation li{
...
.treeview .hover{color:#701132;cursor:pointer;}
.treeview li{}
...
#content ul#navigation li a.count_elements {
    border-bottom: 0px dashed #22396D;
}
#content ul#navigation li a.go_to_link {
    border-bottom: 0px dashed #22396D;
}
.ie7 .ie_count{
...
foreach($arResult["ITEMS"] as $arSection):?>
	...
	<? if ($arSection["ELEMENT_CNT"] !== "0"){?>		
		...
		<?endif?>
		<?if (!$bFlagIsCourse):?>
			<li id="cat_<?=$arSection['ID']?>" class="<?if ($arSection["DEPTH_LEVEL"] > 1){?>closed<? } else {?>closed<? } ?>"><div class="main-course-link"><a href="<?if ($arSection["ID"]!=541) {echo $arSection["DETAIL_PAGE_URL"];} else {echo "javascript:void(0)";}?>" class="<?if ($arSection["DEPTH_LEVEL"] === "1"){?>first_one <? } else {?>twoplus <? } ?><?if ($pos !== false) {?>tr_school<? } ?>"><?=$arSection["NAME"]?></a></div>
               <?if ($arSection["ID"]!=541) {?><a href="<?=$arSection["DETAIL_PAGE_URL"]?>"><div class="double-arrow" <?if ($arSection["DEPTH_LEVEL"] > 1){?>style="background-position: 0 4px"<?}?>></div></a><?}?>
					<?if ($pos !== false) {?>
			...
			<li>
				<?if ($bFlagIsCourse){?>
					<div class="clases_wrapper">
						<div class="l <?if ($arSection["DEPTH_LEVEL"] == 4){?>w425<?}else {?>w545<? } ?>"><?=$arSection['PROPERTY_PP_COURSE_CODE']?> <a class="goaway" title="" href="/kurs/<?=$arSection['PROPERTY_PP_COURSE_XML_ID']?>.html"><?=$arSection['PROPERTY_PP_COURSE_NAME']?></a></div>
