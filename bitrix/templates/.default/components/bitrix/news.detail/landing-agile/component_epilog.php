...
<?
	...
	$GLOBALS["arrFilter"] = array(
					...
					"PROPERTY_course_code"=> array("SDP-031", "SDP-032", "SDP-033", "SDP-034", "SDP-035","SDP-045")
				);
						if (strlen($_REQUEST["qcat"])>0) {
                            $GLOBALS["arrFilter"][]=array("LOGIC"=>"OR", array("NAME"=> "%".$_REQUEST["qcat"]."%"));

	}
	$GLOBALS["linkSchedule"] = $arResult["PROPERTIES"]["LINK_SCHEDULE"]["VALUE"];
?>


<?
$APPLICATION->IncludeComponent("bitrix:news.list", "edu_ru_all_city_agile_land", array(
...
?>
        ...
        <div class="map" id="s5">
            <div class="info">
                ...
                <pre style="display: none"><?=var_export($arResult)?></pre>
    ...
    <ul class="social">
        ...
        <li><a class="facebook" target="_blank" href="https://www.facebook.com/sharer/sharer.php?u=<?=urlencode("http://www.luxoft-training.ru".$APPLICATION->GetCurDir())?>">facebook</a></li>
	...
	array(
		...
		"SHOW_CITIES" => "N",
		"COMPONENT_TEMPLATE" => "landing-agile",
        "DISCOUNT_TIME" => $GLOBALS['DISCOUNT_TIME']
	),
	false
);?>
    </div>
