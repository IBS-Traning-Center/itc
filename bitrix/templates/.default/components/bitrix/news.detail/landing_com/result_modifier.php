...
<?
if (count($arResult["PROPERTIES"]["REASONS"]["VALUE"])==3) {
	$arSelect = Array("NAME", "PREVIEW_TEXT");
	$arFilter = Array("IBLOCK_ID"=>127, "ID"=>$arResult["PROPERTIES"]["REASONS"]["VALUE"], "ACTIVE"=>"Y");
...
if (intval($arResult["PROPERTIES"]["FIRST_PRICE"]["VALUE"])>0 && intval($arResult["PROPERTIES"]["SECOND_PRICE"]["VALUE"])>0 && intval($arResult["PROPERTIES"]["THIRD_PRICE"]["VALUE"])>0 && intval($arResult["PROPERTIES"]["FOURTH_PRICE"]["VALUE"])>0) {
	...
	$arResult["ETAPS_COUNT"]=1;
		  ...
		  $course_tran_topics_html_text = $arCourse['PROPERTIES']['HTML_ROADMAP']['VALUE']['TEXT'];
          $course_tran_topics_html_type = $arCourse['PROPERTIES']['HTML_ROADMAP']['VALUE']['TYPE'];
