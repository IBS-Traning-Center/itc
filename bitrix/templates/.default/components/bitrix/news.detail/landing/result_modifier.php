...
$arResult["COURSE"]["course_audience"] = $arResult['PROPERTIES']['course_audience']['~VALUE'];
GLOBAL $USER;
...
if (count($arResult["PROPERTIES"]["PARTNERS"]["VALUE"]) >= 1) {
    ...
    while ($ob = $res->GetNextElement()) {
        $item = $ob->GetFields();
        $arResult["PARTNERS"][] = array("NAME" => $item["NAME"], "PREVIEW_PICTURE" => CFile::GetFileArray($item["PREVIEW_PICTURE"]), "LINK" => $item["PROPERTY_LINK_VALUE"]);
    }
}
