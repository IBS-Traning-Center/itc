...
<? if ($FIELD !== "---"){?>
		<tr <? if ($FIELD == $tempVaraible){?>style="display:none;"<? }?> >
	...
	switch ($FIELD)
	{
		...
		default:
			...
			?><input size="30" type="text" name="REGISTER[<?=$FIELD?>]" value="<?=$arResult["VALUES"][$FIELD]?><? if ($FIELD == $tempVaraible){?>123<? } ?>" /><?
...
?>
	...
	<tfoot>
		<tr>
			<td></td>
			<td>
			<input type="submit" name="register_submit_button" value="<?=GetMessage("AUTH_REGISTER")?>" /></td>
		</tr>
	</tfoot>
</table>
