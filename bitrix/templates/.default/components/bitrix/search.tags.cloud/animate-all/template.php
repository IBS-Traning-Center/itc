...
<canvas width="275" height="275" id="myCanvas">
	...
	<?foreach ($arResult["SEARCH"] as $key => $res)
		{?>
		<a href="<?=$res["URL"]?>" target="_blank" data-size="<?=$res["FONT_SIZE"]?>" style="font-size: <?=$res["FONT_SIZE"]?>px; color: #<?=$res["COLOR"]?>;" rel="nofollow"><?=$res["NAME"]?></a>
	<?}?>
	</div>
	<div id="tags" <?=$arParams["WIDTH"]?> ><ul style="display: none;"><?
		/*
		foreach ($arResult["SEARCH"] as $key => $res)
		{
