				...
				<div class="order-item">
					<div class="order-title">
						...
						<div class="order-title-inner">
							...
							<a title="<?echo GetMessage("STPOL_DETAIL")?>" class="more_info" href="<?=$val["ORDER"]["URL_TO_DETAIL"] ?>"><?echo GetMessage("STPOL_DETAIL")?></a>
						</div>
					</div>
					<div class="order-info">
						<div class="order-details">
							<div class="order-props">
                                ...
                                <?if ((SITE_ID=="sl") and ($val["ORDER"]["PAY_SYSTEM_ID"] == PAYMENT_SYSTEM_ID_SL_OFERTA_LEGAL)){?>

<blockquote>
<ul>
<li>&bull; необходимо оплатить <a href="/personal/order/payment/?ORDER_ID=<?=intval($val['ORDER']['ID'])?>">счет-оферту</a> (1), а так же сообщить об оплате на <a href="education@ibs.ru">education@ibs.ru</a></li>
<li>&bull;  <a href="/personal/order/payment/?ORDER_ID=<?=intval($val['ORDER']['ID'])?>&SHOWPODPIS=N">cчет-оферту (без подписей со стороны УЦ Luxoft)</a> (2) необходимо распечатать, подписать, поставить печать и направить оригиналы документов в 2-х экземплярах (с указанием обратного адреса, контактного лица) по адресу 127018, Москва, ул. Складочная, д. 3, стр. 1. Твердохлебовой Анне.
<li>&bull; по окончании конференции будут выставлены акты, счет-фактура и направлены вам вместе с подписанным счетом-офертой
<li>&bull; после подписания вами закрывающих документов, акт необходимо направить по адресу 127018, Москва, ул. Складочная, д. 3, стр. 1. Твердохлебовой Анне
</ul>
<span class="links">Сам договор-оферта доступен по <a href="/oferta_sl_legal.doc">ссылке</a></span>
 </blockquote>
...
?>
										<li>
											...
											echo " ";
											echo "<span class='basket_item_name black_color'>".$vvval["NAME"]."</span>";
											if (strlen($vvval["DETAIL_PAGE_URL"]) > 0)  {}
												//echo "</a><br />";
											if($vvval["QUANTITY"] > 1)
						...
						<div class="order-status-info">
                            ...
                            <?//iwrite($val["ORDER"]);
                            ?>
							<?if (($val["ORDER"]["PERSON_TYPE_ID"] == PAYMENT_TYPE_ID_SL_INDIVIDUAL) and ($val["ORDER"]["ID"] > 82)){ ?>
								<a target="_blank" href="/personal/order/print_oferta.php?doc=invoice_oferta_softlabs&ORDER_ID=<?=$val["ORDER"]["ID"]?>" class="orange">См. счет-оферту</a>
							<? } ?>
