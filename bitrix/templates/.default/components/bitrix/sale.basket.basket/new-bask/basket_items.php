...
if ($normalCount > 0):
?>
<div class="frame" id="basket_items_list">
	<div class="clearfix">
		<div class="basket-heading">
			Выбранные тесты (<?=count($arResult["GRID"]["ROWS"])?>)
		</div>
		<div class="basket-right">
			<div class="summ">Сумма:</div> <div id="allSum_FORMATED_2" class="summ-line"><?=str_replace(" ", "&nbsp;", $arResult["allSum_FORMATED"])?></div>
			<a onclick="checkOut();" class="btn btn-buy">Оформить</a>
		</div>
	</div>
	<table class="basket-grid-table" id="basket_items" style="width: 100%;">
			<tr class="heading clearfix">
					...
					foreach ($arResult["GRID"]["HEADERS"] as $id => $arHeader):
						...
						if ($arHeader["id"] == "NAME"):
						?>
							<td class="name-col" id="col_<?=$arHeader["id"];?>">
						<?
						elseif ($arHeader["id"] == "PRICE"):
						?>
							<?/*<td class="stoim-col" id="col_<?=$arHeader["id"];?>">*/?>
						<?elseif ($arHeader["id"] == "QUANTITY"):
						?>
							<td class="quant-col"  id="col_<?=$arHeader["id"];?>">
			...
			<tbody>
				...
				?>
					<tr class="bask-line" id="<?=$arItem["ID"]?>">
						<td class="action-col"><a class="delete-icon" href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delete"])?>"></a></div>
						<?
						foreach ($arResult["GRID"]["HEADERS"] as $id => $arHeader):
							...
							<?
													...
													?>
								<td class="quant-col clearfix">
									<a onclick="setQuantity(<?=$arItem["ID"]?>, <?=$arItem["MEASURE_RATIO"]?>, 'down', <?=$useFloatQuantityJS?>);" href="#">-</a><input value="<?=$arItem["QUANTITY"]?>" id="QUANTITY_INPUT_<?=$arItem["ID"]?>" name="QUANTITY_INPUT_<?=$arItem["ID"]?>"onchange="updateQuantity('QUANTITY_INPUT_<?=$arItem["ID"]?>', '<?=$arItem["ID"]?>', <?=$ratio?>, <?=$useFloatQuantityJS?>)" type="text"/><a onclick="setQuantity(<?=$arItem["ID"]?>, <?=$arItem["MEASURE_RATIO"]?>, 'up', <?=$useFloatQuantityJS?>);" href="#">+</a>
		...
		<div class="clearfix buy-next">
			<a onclick="checkOut();" class="btn btn-buy">Купить</a>
			...
			<table class="bx_ordercart_order_sum">
				...
				<?endif;?>
					...
					<tr>
						...
						<td class="fwb" id="allSum_FORMATED" ><?=str_replace(" p.", "&nbsp;<span>р.</span>", $arResult["allSum_FORMATED"])?></td>
					</tr>
					<tr>
