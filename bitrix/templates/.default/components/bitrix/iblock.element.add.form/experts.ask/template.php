...
?>
	<?=bitrix_sessid_post()?>
<!--
	<input type="hidden" value="не указано" size="25" name="PROPERTY[NAME][0]">
-->
	...
	<table class="data-table">
		...
		<tbody>
			<?foreach ($arResult["PROPERTY_LIST"] as $propertyID):?>
				<tr <?if ($propertyID === "NAME"){?>style="display:none;"<? } ?>>
					...
					<td>
						...
						<textarea cols="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>" rows="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"><?=$value?></textarea>
							...
							case "N":
								...
								?>
								<?if ($propertyID === "NAME"){?>
								<input type="hidden" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" size="25" value="Вопрос от <?echo date(date("d.m.Y H:i:s"))?>" /><br />
								<? } ?>
								<?if ($propertyID !== "NAME"){?>
					...
					[<a href="<?=$arResult["ELEMENT_FILES"][$value]["SRC"]?>"><?=GetMessage("IBLOCK_FORM_FILE_DOWNLOAD")?></a>]<br />
							...
							case "L":
								...
								if ($arResult["PROPERTY_LIST_FULL"][$propertyID]["LIST_TYPE"] == "C")
									...
									$type = $arResult["PROPERTY_LIST_FULL"][$propertyID]["MULTIPLE"] == "Y" ? "dropdown" : "dropdown";
			...
			<?if($arParams["USE_CAPTCHA"] == "Y" && $arParams["ID"] <= 0):?>
				<tr>
					...
					<td>
						...
						<img style="margin: 2px 0 15px 10px; display:block;" src="/bitrix/tools/captcha.php?captcha_sid=<?=$arResult["CAPTCHA_CODE"]?>" width="180" height="40" alt="CAPTCHA" />
					</td>
				</tr>
				<tr>
		...
		<tfoot>
			<tr>
				<td colspan="2">
					<input type="submit" name="iblock_submit" value="Задать вопрос" />
