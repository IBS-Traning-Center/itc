...
?>
	...
	<div class="blog-posts">
		...
		<?if (count($mas)>0 && in_array(75, $mas)) {
			$arBlog = CBlog::GetByUrl($arResult["VARIABLES"]["blog"]);
			if ($_REQUEST["loginby"]=="Y") {
				GLOBAL $USER;
				//$USER->Logout();
				//print_r($arBlog["OWNER_ID"]);
				$USER->Authorize($arBlog["OWNER_ID"]);
				LocalRedirect($APPLICATION->GetCurDir());
			}?>
			<a href="<?=$APPLICATION->GetCurDir()?>?loginby=Y">Залогиниться под создателем блога</a>
		<?}?>	
